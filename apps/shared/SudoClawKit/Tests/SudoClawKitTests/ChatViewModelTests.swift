...
import Foundation
import Testing
@testable import SudoClawChatUI
...
private final class TestChatTransport: @unchecked Sendable, SudoClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SudoClawChatHistoryPayload]
    private let sessionsResponses: [SudoClawChatSessionsListResponse]

    private let stream: AsyncStream<SudoClawChatTransportEvent>
    private let continuation: AsyncStream<SudoClawChatTransportEvent>.Continuation

    init(
        historyResponses: [SudoClawChatHistoryPayload],
        sessionsResponses: [SudoClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SudoClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SudoClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SudoClawChatHistoryPayload(
        ...
        attachments _: [SudoClawChatAttachmentPayload]) async throws -> SudoClawChatSendResponse
    {
        ...
        return SudoClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SudoClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SudoClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SudoClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = SudoClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = SudoClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { SudoClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = SudoClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = SudoClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = SudoClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { SudoClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
