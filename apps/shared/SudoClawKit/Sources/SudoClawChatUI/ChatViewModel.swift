...
private let chatUILogger = Logger(subsystem: "ai.sudoclaw", category: "SudoClawChatUI")

@MainActor
@Observable
public final class SudoClawChatViewModel {
    public private(set) var messages: [SudoClawChatMessage] = []
    ...
    public var attachments: [SudoClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SudoClawChatPendingToolCall] = []
    public private(set) var sessions: [SudoClawChatSessionEntry] = []
    private let transport: any SudoClawChatTransport
    ...
    private var pendingToolCallsById: [String: SudoClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SudoClawChatTransport) {
    ...
    public var sessionChoices: [SudoClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [SudoClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SudoClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SudoClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SudoClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: SudoClawChatMessage) -> SudoClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> SudoClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return SudoClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: SudoClawChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [SudoClawChatMessage],
        incoming: [SudoClawChatMessage]) -> [SudoClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [SudoClawChatMessage]) -> [SudoClawChatMessage] {
        var result: [SudoClawChatMessage] = []
    ...
    private static func dedupeKey(for message: SudoClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SudoClawChatMessageContent] = [
            SudoClawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> SudoClawChatAttachmentPayload in
            SudoClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SudoClawChatSessionEntry {
        SudoClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SudoClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SudoClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SudoClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SudoClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> SudoClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
