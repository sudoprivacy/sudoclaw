...
import "@a2ui/lit/ui";
import { themeContext } from "@sudoclaw/a2ui-theme-context";
...
class SudoClawA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--sudoclaw-a2ui-inset-top, 0px)
        var(--sudoclaw-a2ui-inset-right, 0px)
        var(--sudoclaw-a2ui-inset-bottom, 0px)
        var(--sudoclaw-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--sudoclaw-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--sudoclaw-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--sudoclaw-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--sudoclaw-a2ui-empty-top, var(--sudoclaw-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.sudoclawA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["sudoclaw:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__sudoclawLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.sudoclawCanvasA2UIAction ??
      globalThis.sudoclawCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.sudoclawCanvasA2UIAction) {
...
if (!customElements.get("sudoclaw-a2ui-host")) {
  customElements.define("sudoclaw-a2ui-host", SudoClawA2UIHost);
}
