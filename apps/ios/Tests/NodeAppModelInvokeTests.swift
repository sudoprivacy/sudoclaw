...
private final class MockWatchMessagingService: @preconcurrency WatchMessagingServicing, @unchecked Sendable {
    ...
    var sendError: Error?
    var lastSent: (id: String, params: SudoClawWatchNotifyParams)?
    ...
    func sendNotification(id: String, params: SudoClawWatchNotifyParams) async throws -> WatchNotificationSendResult {
...
@Suite(.serialized) struct NodeAppModelInvokeTests {
    @Test @MainActor func decodeParamsFailsWithoutJSON() {
        #expect(throws: Error.self) {
            _ = try NodeAppModel._test_decodeParams(SudoClawCanvasNavigateParams.self, from: nil)
        }
    }
    ...
    @Test @MainActor func handleInvokeRejectsBackgroundCommands() async {
        ...
        let req = BridgeInvokeRequest(id: "bg", command: SudoClawCanvasCommand.present.rawValue)
    ...
    @Test @MainActor func handleInvokeRejectsCameraWhenDisabled() async {
        let appModel = NodeAppModel()
        let req = BridgeInvokeRequest(id: "cam", command: SudoClawCameraCommand.snap.rawValue)
    ...
    @Test @MainActor func handleInvokeRejectsInvalidScreenFormat() async {
        let appModel = NodeAppModel()
        let params = SudoClawScreenRecordParams(format: "gif")
        let data = try? JSONEncoder().encode(params)
        let json = data.flatMap { String(data: $0, encoding: .utf8) }

        let req = BridgeInvokeRequest(
            id: "screen",
            command: SudoClawScreenCommand.record.rawValue,
            paramsJSON: json)
    ...
    @Test @MainActor func handleInvokeCanvasCommandsUpdateScreen() async throws {
        ...
        let present = BridgeInvokeRequest(id: "present", command: SudoClawCanvasCommand.present.rawValue)
        ...
        let navigateParams = SudoClawCanvasNavigateParams(url: "http://example.com/")
        let navData = try JSONEncoder().encode(navigateParams)
        let navJSON = String(decoding: navData, as: UTF8.self)
        let navigate = BridgeInvokeRequest(
            id: "nav",
            command: SudoClawCanvasCommand.navigate.rawValue,
            paramsJSON: navJSON)
        let navRes = await appModel._test_handleInvoke(navigate)
        #expect(navRes.ok == true)
        #expect(appModel.screen.urlString == "http://example.com/")

        let evalParams = SudoClawCanvasEvalParams(javaScript: "1+1")
        ...
        let eval = BridgeInvokeRequest(
            id: "eval",
            command: SudoClawCanvasCommand.evalJS.rawValue,
            paramsJSON: evalJSON)
    ...
    @Test @MainActor func handleInvokeA2UICommandsFailWhenHostMissing() async throws {
        ...
        let reset = BridgeInvokeRequest(id: "reset", command: SudoClawCanvasA2UICommand.reset.rawValue)
        ...
        let pushParams = SudoClawCanvasA2UIPushJSONLParams(jsonl: jsonl)
        ...
        let push = BridgeInvokeRequest(
            id: "push",
            command: SudoClawCanvasA2UICommand.pushJSONL.rawValue,
            paramsJSON: pushJSON)
    ...
    @Test @MainActor func handleInvokeWatchStatusReturnsServiceSnapshot() async throws {
        ...
        let req = BridgeInvokeRequest(id: "watch-status", command: SudoClawWatchCommand.status.rawValue)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)

        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(SudoClawWatchStatusPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRoutesToWatchService() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = SudoClawWatchNotifyParams(
            title: "SudoClaw",
        ...
        let req = BridgeInvokeRequest(
            id: "watch-notify",
            command: SudoClawWatchCommand.notify.rawValue,
        ...
        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(SudoClawWatchNotifyPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRejectsEmptyMessage() async throws {
        let watchService = MockWatchMessagingService()
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = SudoClawWatchNotifyParams(title: "   ", body: "\n")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyDerivesPriorityFromRiskAndCapsActions() async throws {
        ...
        let params = SudoClawWatchNotifyParams(
            ...
            risk: .high,
            actions: [
                SudoClawWatchAction(id: "a1", label: "A1"),
                SudoClawWatchAction(id: "a2", label: "A2"),
                SudoClawWatchAction(id: "a3", label: "A3"),
                SudoClawWatchAction(id: "a4", label: "A4"),
                SudoClawWatchAction(id: "a5", label: "A5"),
            ])
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyReturnsUnavailableOnDeliveryFailure() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = SudoClawWatchNotifyParams(title: "SudoClaw", body: "Delivery check")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleDeepLinkSetsErrorWhenNotConnected() async {
        let appModel = NodeAppModel()
        let url = URL(string: "sudoclaw://agent?message=hello")!
    ...
    @Test @MainActor func handleDeepLinkRejectsOversizedMessage() async {
        ...
        let url = URL(string: "sudoclaw://agent?message=\(msg)")!
        await appModel.handleDeepLink(url: url)
