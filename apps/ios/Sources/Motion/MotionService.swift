...
final class MotionService: MotionServicing {
    func activities(params: SudoClawMotionActivityParams) async throws -> SudoClawMotionActivityPayload {
        ...
        let mapped: [SudoClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return SudoClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: SudoClawPedometerParams) async throws -> SudoClawPedometerPayload {
        ...
        let payload: SudoClawPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = SudoClawPedometerPayload(
