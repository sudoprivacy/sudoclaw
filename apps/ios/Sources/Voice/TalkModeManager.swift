...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__SUDOCLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<SudoClawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.sudoclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = SudoClawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> SudoClawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return SudoClawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> SudoClawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> SudoClawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return SudoClawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> SudoClawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = SudoClawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: SudoClawTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: SudoClawAgentEventPayload.self
            ) else {
                continue
            }
