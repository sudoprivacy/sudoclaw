...
final class ContactsService: ContactsServicing {
    ...
    func search(params: SudoClawContactsSearchParams) async throws -> SudoClawContactsSearchPayload {
        ...
        return SudoClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: SudoClawContactsAddParams) async throws -> SudoClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return SudoClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return SudoClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> SudoClawContactPayload {
        SudoClawContactPayload(
