...
struct IOSGatewayChatTransport: SudoClawChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.sudoclaw", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> SudoClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(SudoClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SudoClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(SudoClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [SudoClawChatAttachmentPayload]) async throws -> SudoClawChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [SudoClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(SudoClawChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(SudoClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SudoClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SudoClawChatEventPayload.self)
                        {
