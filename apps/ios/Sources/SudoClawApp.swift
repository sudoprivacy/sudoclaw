...
@MainActor
final class SudoClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.sudoclaw.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.sudoclaw.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.sudoclaw.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "sudoclaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "sudoclaw.watch.promptId"
    static let sessionKeyKey = "sudoclaw.watch.sessionKey"
    static let actionPrimaryIDKey = "sudoclaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "sudoclaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "sudoclaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "sudoclaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "sudoclaw.watch.action.primary"
    static let actionSecondaryIdentifier = "sudoclaw.watch.action.secondary"
    static let actionIdentifierPrefix = "sudoclaw.watch.action."
    static let actionIDKeyPrefix = "sudoclaw.watch.action.id."
    static let actionLabelKeyPrefix = "sudoclaw.watch.action.label."
    static let categoryPrefix = "sudoclaw.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: SudoClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> SudoClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return SudoClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "SudoClaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [SudoClawWatchAction]) -> [UNNotificationAction] {
...
@main
struct SudoClawApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(SudoClawAppDelegate.self) private var appDelegate
...
extension SudoClawApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("SudoClaw: installing uncaught exception handler")
