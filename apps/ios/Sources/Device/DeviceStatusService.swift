...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> SudoClawDeviceStatusPayload {
        ...
        return SudoClawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> SudoClawDeviceInfoPayload {
        ...
        return SudoClawDeviceInfoPayload(
    ...
    private func batteryStatus() -> SudoClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: SudoClawBatteryState = switch device.batteryState {
        ...
        return SudoClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> SudoClawThermalStatusPayload {
        let state: SudoClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return SudoClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> SudoClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return SudoClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
