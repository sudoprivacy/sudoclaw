...
final class RemindersService: RemindersServicing {
    func list(params: SudoClawRemindersListParams) async throws -> SudoClawRemindersListPayload {
        ...
        let payload: [SudoClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return SudoClawReminderPayload(
        ...
        return SudoClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: SudoClawRemindersAddParams) async throws -> SudoClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = SudoClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return SudoClawRemindersAddPayload(reminder: payload)
    }
