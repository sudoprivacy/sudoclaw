...
struct OnboardingWizardView: View {
    ...
    private var welcomeStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Connect to your SudoClaw gateway")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                ...
                Section {
                    ...
                    Text(
                        "Approve this device on the gateway.\n"
                            ...
                            + "\(requestLine)\n"
                            + "SudoClaw will also retry automatically when you return to this app.")
                }
            }
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            if let last = GatewaySettingsStore.loadLastGatewayConnection() {
                ...
                case .discovered:
                    self.manualHost = "sudoclaw.local"
        ...
        if self.selectedMode == .developerLocal && self.manualHost == "sudoclaw.local" {
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "sudoclaw.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "sudoclaw.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "sudoclaw.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
