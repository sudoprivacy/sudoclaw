...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SudoClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.sudoclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SudoClawKit.AnyCodable]()) {
                $0[$1.key] = SudoClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SudoClawProtocol.AnyCodable?) -> [String: SudoClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SudoClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SudoClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SudoClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("sudoclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("sudoclaw.control.agent")
}
