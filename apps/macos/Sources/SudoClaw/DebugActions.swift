...
enum DebugActions {
    private static let verboseDefaultsKey = "sudoclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "sudoclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = SudoClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "SudoClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = SudoClawPaths.configURL
        guard
