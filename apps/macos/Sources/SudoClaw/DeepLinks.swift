...
private let deepLinkLogger = Logger(subsystem: "ai.sudoclaw", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "SudoClaw is paused", message: "Unpause SudoClaw to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run SudoClaw agent?", message: body) else { return }
        }
