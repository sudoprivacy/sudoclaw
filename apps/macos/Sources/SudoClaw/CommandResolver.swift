...
enum CommandResolver {
    private static let projectRootDefaultsKey = "sudoclaw.gatewayProjectRootPath"
    private static let helperName = "sudoclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let sudoclawEntry = root.appendingPathComponent("sudoclaw.mjs").path
        if FileManager().isReadableFile(atPath: sudoclawEntry) { return sudoclawEntry }
        let binEntry = root.appendingPathComponent("bin/sudoclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/sudoclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let sudoclawPaths = self.sudoclawManagedPaths(home: home)
        if !sudoclawPaths.isEmpty {
            extras.insert(contentsOf: sudoclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + sudoclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func sudoclawManagedPaths(home: URL) -> [String] {
        let bases = [
    ...
    static func sudoclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSudoClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("sudoclaw.mjs").path,
            root.appendingPathComponent("bin/sudoclaw.js").path,
        ]
    ...
    static func hasAnySudoClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.sudoclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func sudoclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let sudoclawPath = self.projectSudoClawExecutable(projectRoot: root) {
            return [sudoclawPath, subcommand] + extraArgs
        }
        if let sudoclawPath = self.sudoclawExecutable(searchPaths: searchPaths) {
            return [sudoclawPath, subcommand] + extraArgs
        }
        ...
        case .success:
            let missingEntry = """
            sudoclaw entrypoint missing (looked for dist/index.js or sudoclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real sudoclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/sudoclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v sudoclaw >/dev/null 2>&1; then
          CLI="$(command -v sudoclaw)"
          sudoclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/sudoclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/sudoclaw.mjs"
            node "$PRJ/sudoclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/sudoclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/sudoclaw.js"
            node "$PRJ/bin/sudoclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent sudoclaw"
          pnpm --silent sudoclaw \(quotedArgs);
        else
          echo "sudoclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SudoClawConfigFile.loadDict()
