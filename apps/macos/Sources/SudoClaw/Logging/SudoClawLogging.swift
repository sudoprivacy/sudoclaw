...
enum SudoClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SudoClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SudoClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SudoClawLogging.bootstrapIfNeeded()
        let label = SudoClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SudoClawOSLogHandler: LogHandler {
...
struct SudoClawFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SudoClawLogging.parseLabel(self.label)
