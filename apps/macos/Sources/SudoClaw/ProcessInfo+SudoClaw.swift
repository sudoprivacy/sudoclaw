...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.sudoclaw.mac`) even if the shipped
    ...
    {
        if environment["SUDOCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "sudoclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "sudoclaw.nixMode") { return true }
