...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.sudoclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SudoClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SudoClawSessionPreviewEntry,
    ...
    private static func cache(payload: SudoClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SudoClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SudoClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SudoClawKit.AnyCodable]) -> [SudoClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SudoClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SudoClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SudoClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SudoClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SudoClawChatMessage) -> String? {
