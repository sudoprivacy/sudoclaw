...
extension OnboardingView {
    ...
    func welcomePage() -> some View {
        self.onboardingPage {
            VStack(spacing: 22) {
                Text("Welcome to SudoClaw")
                    .font(.largeTitle.weight(.semibold))
                Text("SudoClaw is a powerful personal AI assistant that can connect to WhatsApp or Telegram.")
                    .font(.body)
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Text(
                                "The connected AI agent (e.g. Claude) can trigger powerful actions on your Mac, " +
                                    ...
                                    "Only enable SudoClaw if you understand the risks and trust the prompts and " +
    ...
    func connectionPage() -> some View {
        self.onboardingPage {
            Text("Choose your Gateway")
                .font(.largeTitle.weight(.semibold))
            Text(
                "SudoClaw uses a single Gateway that stays running. Pick this Mac, " +
                    ...
                    if self.state.remoteTransport == .ssh {
                        ...
                        GridRow {
                            Text("Project root")
                                .font(.callout.weight(.semibold))
                                .frame(width: labelWidth, alignment: .leading)
                            TextField("/home/you/Projects/sudoclaw", text: self.$state.remoteProjectRoot)
                                .textFieldStyle(.roundedBorder)
                                .frame(width: fieldWidth)
                        }
                        GridRow {
                            Text("CLI path")
                                ...
                                .frame(width: labelWidth, alignment: .leading)
                            TextField(
                                "/Applications/SudoClaw.app/.../sudoclaw",
    ...
    func permissionsPage() -> some View {
        self.onboardingPage {
            Text("Grant permissions")
                .font(.largeTitle.weight(.semibold))
            Text("These macOS permissions let SudoClaw automate apps and capture context on this Mac.")
                .font(.body)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 520)
    ...
    func cliPage() -> some View {
        self.onboardingPage {
            Text("Install the CLI")
                .font(.largeTitle.weight(.semibold))
            Text("Required for local mode: installs `sudoclaw` so launchd can run the gateway.")
                .font(.body)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 520)
    ...
    func workspacePage() -> some View {
        self.onboardingPage {
            Text("Agent workspace")
                .font(.largeTitle.weight(.semibold))
            Text(
                "SudoClaw runs the agent from a dedicated workspace so it can load `AGENTS.md` " +
                ...
                if self.state.connectionMode == .remote {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Workspace folder")
                            .font(.headline)
                        TextField(
                            AgentWorkspace.displayPath(for: SudoClawConfigFile.defaultWorkspaceURL()),
                                ...
                                Task {
                                    ...
                                    if saved {
                                        self.workspaceStatus =
                                            "Saved to ~/.sudoclaw/sudoclaw.json (agents.defaults.workspace)"
                                    }
                                }
                            }
                            .buttonStyle(.bordered)
    ...
    func onboardingChatPage() -> some View {
        VStack(spacing: 16) {
            ...
            self.onboardingGlassCard(padding: 8) {
                SudoClawChatView(viewModel: self.onboardingChatModel, style: .onboarding)
            ...
            self.onboardingCard {
                ...
                if self.state.connectionMode == .remote {
                    self.featureRow(
                        ...
                        subtitle: """
                        On your gateway host: install/update the `sudoclaw` package and make sure credentials exist
                        (typically `~/.sudoclaw/credentials/oauth.json`). Then connect again if needed.
                        """,
                        systemImage: "network")
                    Divider()
                        .padding(.vertical, 6)
                ...
                self.featureRow(
                    ...
                    subtitle: "Click the SudoClaw menu bar icon for quick chat and status.",
