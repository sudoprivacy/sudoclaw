...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.sudoclaw", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: SudoClawNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [SudoClawCapability.canvas.rawValue, SudoClawCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(SudoClawCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if SudoClawLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(SudoClawCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            SudoClawSystemCommand.which.rawValue,
            SudoClawSystemCommand.run.rawValue,
            SudoClawSystemCommand.execApprovalsGet.rawValue,
            SudoClawSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(SudoClawCapability.camera.rawValue) {
        ...
        if capsSet.contains(SudoClawCapability.location.rawValue) {
