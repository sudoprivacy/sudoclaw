...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.sudoclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: SudoClawPeekabooBridgeServices?
    private static var sudoclawSocketPath: String {
    ...
    private func startIfNeeded() async {
        ...
        let services = SudoClawPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.sudoclawSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.sudoclawSocketPath, privacy: .public)")
    }
...
@MainActor
private final class SudoClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.sudoclaw.peekaboo")
