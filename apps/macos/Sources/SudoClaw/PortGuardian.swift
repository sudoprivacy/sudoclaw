...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.sudoclaw", category: "portguard")
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "sudoclaw", "tsx", "pnpm", "bun"]
    ...
    private func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .remote:
            ...
            return false
        case .local:
            // The gateway daemon may listen as `sudoclaw` or as its runtime (`node`, `bun`, etc).
            if full.contains("gateway-daemon") { return true }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("sudoclaw"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
