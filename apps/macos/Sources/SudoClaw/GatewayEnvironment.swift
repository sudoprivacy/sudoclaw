...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.sudoclaw", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["SUDOCLAW_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = SudoClawConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.sudoclawExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "sudoclaw CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "sudoclaw@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "sudoclaw@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "sudoclaw@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "sudoclaw@\(target)"])
            }

        statusHandler("Installing sudoclaw@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed sudoclaw@\(target)")
        } else {
