...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersSudoClawBinary() throws {
        ...
        let sudoclawPath = tmp.appendingPathComponent("node_modules/.bin/sudoclaw")
        try self.makeExec(at: sudoclawPath)

        let cmd = CommandResolver.sudoclawCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([sudoclawPath.path, "gateway"]))
    }
    ...
    @Test func fallsBackToNodeAndScript() throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/sudoclaw.js")
        try self.makeExec(at: nodePath)
        try "#!/bin/sh\necho v22.0.0\n".write(to: nodePath, atomically: true, encoding: .utf8)
        try FileManager().setAttributes([.posixPermissions: 0o755], ofItemAtPath: nodePath.path)
        try self.makeExec(at: scriptPath)

        let cmd = CommandResolver.sudoclawCommand(
            subcommand: "rpc",
            defaults: defaults,
            configRoot: [:],
    ...
    @Test func prefersSudoClawBinaryOverPnpm() throws {
        let defaults = self.makeDefaults()
        defaults.set(AppState.ConnectionMode.local.rawValue, forKey: connectionModeKey)
        ...
        let sudoclawPath = binDir.appendingPathComponent("sudoclaw")
        ...
        #expect(cmd.prefix(2).elementsEqual([sudoclawPath.path, "rpc"]))
    }

    @Test func usesSudoClawBinaryWithoutNodeRuntime() throws {
        let defaults = self.makeDefaults()
        defaults.set(AppState.ConnectionMode.local.rawValue, forKey: connectionModeKey)
    ...
    @Test func buildsSSHCommandForRemoteMode() {
        ...
        defaults.set("sudoclaw@example.com:2222", forKey: remoteTargetKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "sudoclaw@example.com")
        } else {
            #expect(Bool(false))
        }
    ...
    @Test func configRootLocalOverridesRemoteDefaults() throws {
        ...
        #expect(cmd.first == sudoclawPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
