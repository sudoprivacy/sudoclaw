...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: SudoClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SudoClawProtocol.AnyCodable] = [
            "presence": .init([SudoClawProtocol.AnyCodable(entry)]),
        ]
        let payload = SudoClawProtocol.AnyCodable(payloadMap)
