...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/sudoclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/sudoclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/sudoclaw/cron");
  });
  ...
  it("honors explicit base path overrides", async () => {
    ...
    expect(app.basePath).toBe("/sudoclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/sudoclaw/sessions");
  });
