...
export async function handleWhatsAppStart(host: SudoClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: SudoClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: SudoClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: SudoClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: SudoClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: SudoClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: SudoClawApp): string | null {
...
function buildGatewayHttpHeaders(host: SudoClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: SudoClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: SudoClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: SudoClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: SudoClawApp) {
  const state = host.nostrProfileFormState;
