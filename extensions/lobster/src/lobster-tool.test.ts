...
import type { SudoClawPluginApi, SudoClawPluginToolContext } from "../../../src/plugins/types.js";
import {
...
function fakeApi(overrides: Partial<SudoClawPluginApi> = {}): SudoClawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<SudoClawPluginToolContext> = {}): SudoClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: SudoClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
