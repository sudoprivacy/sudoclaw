...
`sudoclaw.invoke --each`) without adding new SudoClaw code per workflow.
...
This extension depends on SudoClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** SudoClaw extension (like `lobster`) and
