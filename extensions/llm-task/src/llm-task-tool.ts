...
import Ajv from "ajv";
import { resolvePreferredSudoClawTmpDir } from "sudoclaw/plugin-sdk";
// NOTE: This extension is intended to be bundled with SudoClaw.
// When running from source (tests/dev), SudoClaw internals live under src/.
...
export function createLlmTaskTool(api: SudoClawPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via sudoclaw.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredSudoClawTmpDir(), "sudoclaw-llm-task-"),
        );
