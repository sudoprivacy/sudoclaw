...
export function createToolFactoryHarness(cfg: SudoClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<SudoClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
