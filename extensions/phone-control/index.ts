...
import type { SudoClawPluginApi, SudoClawPluginService } from "sudoclaw/plugin-sdk";
...
function normalizeDenyList(cfg: SudoClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: SudoClawPluginApi["config"]): string[] {
...
export default function register(api: SudoClawPluginApi) {
  ...
  const timerService: SudoClawPluginService = {
