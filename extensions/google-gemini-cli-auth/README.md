# Google Gemini CLI Auth (SudoClaw plugin)
...
```bash
sudoclaw plugins enable google-gemini-cli-auth
```
...
```bash
sudoclaw models auth login --provider google-gemini-cli --set-default
```
...
- `SUDOCLAW_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `SUDOCLAW_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
