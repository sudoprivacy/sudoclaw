...
function resolveGatewayPort(cfg: SudoClawPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.SUDOCLAW_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: SudoClawPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: SudoClawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the SudoClaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the SudoClaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
