...
function resolveSudoClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.SUDOCLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set SUDOCLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveSudoClawRoot(), "dist", "extensionAPI.js");
