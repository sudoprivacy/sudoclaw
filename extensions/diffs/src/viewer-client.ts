...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-sudoclaw-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-sudoclaw-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.sudoclawDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.sudoclawDiffsError = "true";
