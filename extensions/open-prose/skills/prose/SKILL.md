...
metadata: { "sudoclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == SudoClaw `sessions_spawn`
- **File I/O** == SudoClaw `read`/`write`
- **Remote fetch** == SudoClaw `web_fetch` (or `exec` with curl when POST is required)
