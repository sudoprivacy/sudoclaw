...
vi.mock("sudoclaw/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("sudoclaw/plugin-sdk")>("sudoclaw/plugin-sdk");
  return {
    ...actual,
...
import type { SudoClawPluginServiceContext } from "sudoclaw/plugin-sdk";
import { emitDiagnosticEvent } from "sudoclaw/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/sudoclaw-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): SudoClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("sudoclaw.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("sudoclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("sudoclaw.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("sudoclaw.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("sudoclaw.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("sudoclaw.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("sudoclaw.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("sudoclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("sudoclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("sudoclaw.webhook.processed");
    expect(spanNames).toContain("sudoclaw.message.processed");
    expect(spanNames).toContain("sudoclaw.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["sudoclaw.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("sudoclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "sudoclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["sudoclaw.reason"]).toBe("string");
    expect(String(attrs?.["sudoclaw.reason"])).not.toContain(
