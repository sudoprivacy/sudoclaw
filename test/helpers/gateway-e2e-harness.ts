...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `sudoclaw-e2e-${name}-`));
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          SUDOCLAW_SKIP_PROVIDERS: "1",
          SUDOCLAW_SKIP_GMAIL_WATCHER: "1",
          SUDOCLAW_SKIP_CRON: "1",
          SUDOCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
          SUDOCLAW_SKIP_CANVAS_HOST: "1",
          SUDOCLAW_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
