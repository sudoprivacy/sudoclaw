---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for SudoClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, SudoClaw cron scheduling for periodic checks, or version status checks on a machine running SudoClaw (laptop, workstation, Pi, VPS).
---

# SudoClaw Host Hardening

## Overview

Assess and harden the host running SudoClaw, then align it to a user-defined risk tolerance without breaking access. Use SudoClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim SudoClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. SudoClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if SudoClaw can access sensitive data.
...
Offer to apply SudoClaw safe defaults (numbered):

1. `sudoclaw security audit --fix`

Be explicit that `--fix` only tightens SudoClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check SudoClaw version/update status (read-only)
...
- Credential hygiene notes (location of SudoClaw creds, prefer disk encryption)
...
After SudoClaw install or first hardening pass, run at least one baseline audit and version check:

- `sudoclaw security audit`
- `sudoclaw security audit --deep`
- `sudoclaw update status`

Ongoing monitoring is recommended. Use the SudoClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
2. `npm view sudoclaw version` (published npm version)

## SudoClaw command accuracy
...
Do not invent CLI flags or imply SudoClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by SudoClaw compaction:
