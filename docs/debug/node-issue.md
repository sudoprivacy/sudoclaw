...
```
[sudoclaw] Failed to start CLI: TypeError: __name is not a function
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch sudoclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node sudoclaw.mjs status` works on Node 25.
