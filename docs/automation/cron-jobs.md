...
- Jobs persist under `~/.sudoclaw/cron/` so restarts don’t lose schedules.
...
sudoclaw cron list
sudoclaw cron run <job-id>
sudoclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.sudoclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `sudoclaw cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, SudoClaw applies a
...
If `delivery` is omitted for isolated jobs, SudoClaw defaults to `announce`.
...
- Job store: `~/.sudoclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.sudoclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, SudoClaw classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, SudoClaw archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `sudoclaw cron runs` and adjust retention before logs become large
...
sudoclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
sudoclaw cron edit <jobId> --agent ops
sudoclaw cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
sudoclaw cron run <jobId>
sudoclaw cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
sudoclaw cron edit <jobId> \
...
```bash
sudoclaw cron edit <jobId> --exact
```
...
```bash
sudoclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`sudoclaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SUDOCLAW_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- SudoClaw applies exponential retry backoff for recurring jobs after consecutive errors:
