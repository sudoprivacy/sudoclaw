...
- Publish as `sudoclaw/plugin-sdk` (or export from core under `sudoclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `SudoClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SudoClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SudoClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SudoClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SudoClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SudoClawConfig): string;
  };
};
```
...
- Introduce `sudoclaw/plugin-sdk`.
- Add `api.runtime` to `SudoClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `sudoclawRuntime: ">=2026.2.0"`).
