...
read_when:
  - Designing workspace memory (~/.sudoclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep SudoClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.sudoclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.sudoclaw/workspace` as canonical human-readable memory.
...
```
~/.sudoclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in SudoClaw**, but keep a separable core library.

### Why integrate into SudoClaw?

- SudoClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `sudoclaw memory recall "…" --k 25 --since 30d`
  - `sudoclaw memory reflect --since 7d`
...
Pragmatic take for `~/.sudoclaw/workspace`:
