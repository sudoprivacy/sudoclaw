...
This plan defines how SudoClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- SudoClaw core owns ACP control plane concerns
...
SudoClaw should not reimplement ACP transport internals in core.
SudoClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in SudoClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under SudoClaw state dir
...
- ACP event stream is normalized to SudoClaw reply chunks
...
For the first production backend (`extensions/acpx`), SudoClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses SudoClaw `sessionKey`)
...
- SudoClaw consumes ndjson events from `acpx --format json --json-strict`
