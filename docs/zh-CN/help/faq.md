...
其他有用的 CLI 检查：`openclaw status --all`、`sudoclaw logs --follow`、
...
使用托管覆盖而不是编辑仓库副本。将你的更改放在 `~/.openclaw/skills/<name>/SKILL.md`（或通过 `~/.sudoclaw/sudoclaw.json` 中的 `skills.load.extraDirs` 添加文件夹）。优先级是 `<workspace>/skills` > `~/.openclaw/skills` > 内置，所以托管覆盖优先生效而不会修改 git。只有值得上游合并的编辑才应该放在仓库中并作为 PR 提交。

### 可以从自定义文件夹加载 Skills 吗

可以。通过 `~/.sudoclaw/sudoclaw.json` 中的 `skills.load.extraDirs` 添加额外目录（最低优先级）。默认优先级保持不变：`<workspace>/skills` → `~/.openclaw/skills` → 内置 → `skills.load.extraDirs`。`clawhub` 默认安装到 `./skills`，OpenClaw 将其视为 `<workspace>/skills`。
...
旧版单智能体路径：`~/.openclaw/agent/*`（通过 `sudoclaw doctor` 迁移）。
...
OpenClaw 从 `$OPENCLAW_CONFIG_PATH`（默认：`~/.sudoclaw/sudoclaw.json`）读取可选的 **JSON5** 配置：
...
- 如果你使用了配置文件（`--profile` / `SUDOCLAW_PROFILE`），重置每个状态目录（默认为 `~/.openclaw-<profile>`）。
...
- **确认认证配置文件的位置**（新路径 vs 旧路径）
  ...
  - 旧版：`~/.openclaw/agent/*`（通过 `sudoclaw doctor` 迁移）
