...
```bash
sudoclaw plugins enable minimax-portal-auth  # 如果已加载则跳过
sudoclaw gateway restart  # 如果 Gateway 网关已在运行则重启
...
## 通过 `sudoclaw configure` 配置

使用交互式配置向导设置 MiniMax，无需编辑 JSON：

1. 运行 `sudoclaw configure`。
2. 选择 **Model/auth**。
3. 选择 **MiniMax M2.1**。
4. 在提示时选择你的默认模型。
...
- 使用 `sudoclaw models list` 和 `sudoclaw models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `sudoclaw configure` 并选择 **MiniMax M2.1**，或
