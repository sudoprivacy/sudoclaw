...
你可以通过 `sudoclaw.json` 中的 `tools.allow` / `tools.deny` 全局允许/拒绝工具
...
- `host=node` 可以针对 macOS 配套应用或无头节点主机（`sudoclaw node run`）。
...
- 需要 Brave API 密钥（推荐：`sudoclaw configure --section web`，或设置 `BRAVE_API_KEY`）。
...
- 快速冒烟测试：`sudoclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`。
...
- `restart`（授权 + 发送 `SIGUSR1` 进行进程内重启；`sudoclaw gateway` 就地重启）
