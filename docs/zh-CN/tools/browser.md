...
SudoClaw 可以运行一个由智能体控制的**专用 Chrome/Brave/Edge/Chromium 配置文件**。
...
- `chrome`：到你**系统浏览器**的扩展中继（需要将 SudoClaw 扩展附加到标签页）。

如果你希望默认使用托管模式，请设置 `browser.defaultProfile: "sudoclaw"`。

## 配置

浏览器设置位于 `~/.sudoclaw/sudoclaw.json`。
...
- 如果你覆盖了 Gateway 网关端口（`gateway.port` 或 `SUDOCLAW_GATEWAY_PORT`），派生的浏览器端口会相应调整以保持在同一"系列"中。
- 未设置时，`cdpUrl` 默认为中继端口。
...
- 默认配置文件是 `chrome`（扩展中继）。使用 `defaultProfile: "sudoclaw"` 来使用托管浏览器。
...
如果你的**系统默认**浏览器是基于 Chromium 的（Chrome/Brave/Edge 等），SudoClaw 会自动使用它。设置 `browser.executablePath` 可覆盖自动检测：
...
- **远程 CDP：** 设置 `browser.profiles.<name>.cdpUrl`（或 `browser.cdpUrl`）以附加到远程的基于 Chromium 的浏览器。在这种情况下，SudoClaw 不会启动本地浏览器。
...
SudoClaw 在调用 `/json/*` 端点和连接 CDP WebSocket 时会保留认证信息。建议使用环境变量或密钥管理器存储令牌，而不是将其提交到配置文件中。

## 节点浏览器代理（零配置默认）

如果你在有浏览器的机器上运行**节点主机**，SudoClaw 可以自动将浏览器工具调用路由到该节点，无需任何额外的浏览器配置。这是远程 Gateway 网关的默认路径。
...
[Browserless](https://browserless.io) 是一个托管的 Chromium 服务，通过 HTTPS 暴露 CDP 端点。你可以将 SudoClaw 浏览器配置文件指向 Browserless 区域端点，并使用你的 API 密钥进行认证。
...
SudoClaw 支持多个命名配置文件（路由配置）。配置文件可以是：
...
SudoClaw 还可以通过本地 CDP 中继 + Chrome 扩展驱动**你现有的 Chrome 标签页**（无需单独的"sudoclaw"Chrome 实例）。
...
- 你点击标签页上的 **SudoClaw Browser Relay** 扩展图标来附加（它不会自动附加）。
...
- "加载已解压的扩展程序" → 选择 `sudoclaw browser extension path` 打印的目录
...
如果你看到 `Playwright is not available in this gateway build`，请安装完整的 Playwright 包（不是 `playwright-core`）并重启 Gateway 网关，或者重新安装带浏览器支持的 SudoClaw。
...
要持久化浏览器下载，设置 `PLAYWRIGHT_BROWSERS_PATH`（例如 `/home/node/.cache/ms-playwright`）并确保 `/home/node` 通过 `SUDOCLAW_HOME_VOLUME` 或绑定挂载持久化。参见 [Docker](/install/docker)。
...
- `sudoclaw browser download e12 /tmp/report.pdf`
- `sudoclaw browser waitfordownload /tmp/report.pdf`
- `sudoclaw browser upload /tmp/file.pdf`
...
- `sudoclaw browser set headers --json '{"X-Debug":"1"}'`
...
- **AI 快照（数字 ref）**：`sudoclaw browser snapshot`（默认；`--format ai`）
  - 输出：包含数字 ref 的文本快照。
  - 操作：`sudoclaw browser click 12`、`sudoclaw browser type 23 "hello"`。
  - 内部通过 Playwright 的 `aria-ref` 解析 ref。

- **角色快照（角色 ref 如 `e12`）**：`sudoclaw browser snapshot --interactive`（或 `--compact`、`--depth`、`--selector`、`--frame`）
  - 输出：带有 `[ref=e12]`（和可选的 `[nth=1]`）的基于角色的列表/树。
  - 操作：`sudoclaw browser click e12`、`sudoclaw browser highlight e12`。
...
3. 如果仍然失败：`sudoclaw browser highlight <ref>` 查看 Playwright 定位的目标
4. 如果页面行为异常：
   - `sudoclaw browser errors --clear`
   - `sudoclaw browser requests --filter api --clear`
5. 深度调试：录制 trace：
   - `sudoclaw browser trace start`
   - 重现问题
   - `sudoclaw browser trace stop`（打印 `TRACE:<path>`）
...
- `browser act kind=evaluate` / `sudoclaw browser evaluate` 和 `wait --fn` 在页面上下文中执行任意 JavaScript。提示注入可能会操纵它。如果不需要，请使用 `browser.evaluateEnabled=false` 禁用它。
