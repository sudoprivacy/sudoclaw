...
- 重新运行 `openclaw browser extension install` 以刷新 SudoClaw 状态目录下的已安装文件。
...
CLI 有意**不**打印 `node_modules` 路径。始终先运行 `openclaw browser extension install` 将扩展复制到 SudoClaw 状态目录下的稳定位置。
