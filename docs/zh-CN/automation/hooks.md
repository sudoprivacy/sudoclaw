...
- **Webhooks**：外部 HTTP webhooks，让其他系统触发 SudoClaw 中的工作。参见 [Webhook Hooks](/automation/webhook) 或使用 `sudoclaw webhooks` 获取 Gmail 助手命令。
...
- **💾 session-memory**：当你发出 `/new` 时将会话上下文保存到智能体工作区（默认 `~/.sudoclaw/workspace/memory/`）
- **📝 command-logger**：将所有命令事件记录到 `~/.sudoclaw/logs/commands.log`
...
在新手引导期间（`sudoclaw onboard`），你将被提示启用推荐的 hooks。向导会自动发现符合条件的 hooks 并呈现供选择。
...
2. **托管 hooks**：`~/.sudoclaw/hooks/`（用户安装，跨工作区共享）
...
Hook 包可以附带依赖；它们将安装在 `~/.sudoclaw/hooks/<id>` 下。
...
- **托管 hooks**（`~/.sudoclaw/hooks/`）：跨工作区共享
...
**输出**：`<workspace>/memory/YYYY-MM-DD-slug.md`（默认为 `~/.sudoclaw/workspace`）
