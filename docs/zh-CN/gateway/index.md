...
- 替代旧版 `gateway` 命令。CLI 入口点：`sudoclaw gateway`。
...
# 如果端口被占用，终止监听器然后启动：
sudoclaw gateway --force
# 开发循环（TS 更改时自动重载）：
pnpm gateway:watch
```

- 配置热重载监视 `~/.sudoclaw/sudoclaw.json`（或 `SUDOCLAW_CONFIG_PATH`）。
...
- 默认在 `canvasHost.port`（默认 `18793`）上启动 Canvas 文件服务器，从 `~/.sudoclaw/workspace/canvas` 提供 `http://<gateway-host>:18793/__openclaw__/canvas/`。使用 `canvasHost.enabled=false` 或 `SUDOCLAW_SKIP_CANVAS_HOST=1` 禁用。
...
- 默认需要 Gateway 网关认证：设置 `gateway.auth.token`（或 `SUDOCLAW_GATEWAY_TOKEN`）或 `gateway.auth.password`。客户端必须发送 `connect.params.auth.token/password`，除非使用 Tailscale Serve 身份。
- 向导现在默认生成令牌，即使在 loopback 上也是如此。
- 端口优先级：`--port` > `SUDOCLAW_GATEWAY_PORT` > `gateway.port` > 默认 `18789`。
...
- Linux：`sudoclaw-gateway-<profile>.service`
- Windows：`SudoClaw Gateway (<profile>)`

安装元数据嵌入在服务配置中：

- `SUDOCLAW_SERVICE_MARKER=openclaw`
- `SUDOCLAW_SERVICE_KIND=gateway`
...
```bash
sudoclaw --dev setup
sudoclaw --dev gateway --allow-unconfigured
# 然后定位到 dev 实例：
sudoclaw --dev status
sudoclaw --dev health
```
...
- `SUDOCLAW_STATE_DIR=~/.sudoclaw-dev`
- `SUDOCLAW_CONFIG_PATH=~/.sudoclaw-dev/sudoclaw.json`
- `SUDOCLAW_GATEWAY_PORT=19001`（Gateway 网关 WS + HTTP）
- 浏览器控制服务端口 = `19003`（派生：`gateway.port+2`，仅 loopback）
- `canvasHost.port=19005`（派生：`gateway.port+4`）
- 当你在 `--dev` 下运行 `setup`/`onboard` 时，`agents.defaults.workspace` 默认变为 `~/.sudoclaw/workspace-dev`。

派生端口（经验法则）：

- 基础端口 = `gateway.port`（或 `SUDOCLAW_GATEWAY_PORT` / `--port`）
- 浏览器控制服务端口 = 基础 + 2（仅 loopback）
- `canvasHost.port = 基础 + 4`（或 `SUDOCLAW_CANVAS_HOST_PORT` / 配置覆盖）
- 浏览器配置文件 CDP 端口从 `browser.controlPort + 9 .. + 108` 自动分配（按配置文件持久化）。

每个实例的检查清单：

- 唯一的 `gateway.port`
- 唯一的 `SUDOCLAW_CONFIG_PATH`
- 唯一的 `SUDOCLAW_STATE_DIR`
...
```bash
sudoclaw --profile main gateway install
sudoclaw --profile rescue gateway install
```

示例：

```bash
SUDOCLAW_CONFIG_PATH=~/.sudoclaw/a.json SUDOCLAW_STATE_DIR=~/.sudoclaw-a sudoclaw gateway --port 19001
SUDOCLAW_CONFIG_PATH=~/.sudoclaw/b.json SUDOCLAW_STATE_DIR=~/.sudoclaw-b sudoclaw gateway --port 19002
```
...
- `health` — 完整健康快照（与 `sudoclaw health --json` 形状相同）。
- `status` — 简短摘要。
...
- LaunchAgents 是按用户的，需要已登录的会话；对于无头设置，使用自定义 LaunchDaemon（未随附）。
  - `sudoclaw gateway install` 写入 `~/Library/LaunchAgents/bot.molt.gateway.plist`
    （或 `bot.molt.<profile>.plist`；旧版 `com.openclaw.*` 会被清理）。
  - `sudoclaw doctor` 审计 LaunchAgent 配置，可以将其更新为当前默认值。
...
- 如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 SudoClaw 配置文件服务。
  ...
  - 清理：`sudoclaw gateway uninstall`（当前服务）和 `sudoclaw doctor`（旧版迁移）。
- `gateway install` 在已安装时是无操作的；使用 `sudoclaw gateway install --force` 重新安装（配置文件/env/路径更改）。
...
- 要干净地停止它，使用 `sudoclaw gateway stop`（或 `launchctl bootout gui/$UID/bot.molt.gateway`）。
- 要重启，使用 `sudoclaw gateway restart`（或 `launchctl kickstart -k gui/$UID/bot.molt.gateway`）。
  - `launchctl` 仅在 LaunchAgent 已安装时有效；否则先使用 `sudoclaw gateway install`。
...
`sudoclaw gateway install` 写入用户单元。`sudoclaw doctor` 审计
单元并可以将其更新以匹配当前推荐的默认值。

创建 `~/.config/systemd/user/sudoclaw-gateway[-<profile>].service`：
...
[Service]
ExecStart=/usr/local/bin/sudoclaw gateway --port 18789
Restart=always
RestartSec=5
Environment=SUDOCLAW_GATEWAY_TOKEN=
...
```
systemctl --user enable --now sudoclaw-gateway[-<profile>].service
```

**替代方案（系统服务）** - 对于常驻或多用户服务器，你可以
安装 systemd **系统**单元而不是用户单元（无需 lingering）。
创建 `/etc/systemd/system/sudoclaw-gateway[-<profile>].service`（复制上面的单元，
切换 `WantedBy=multi-user.target`，设置 `User=` + `WorkingDirectory=`），然后：

```
sudo systemctl daemon-reload
sudo systemctl enable --now sudoclaw-gateway[-<profile>].service
```
...
- `sudoclaw gateway health|status` — 通过 Gateway 网关 WS 请求 health/status。
- `sudoclaw message send --target <num> --message "hi" [--media ...]` — 通过 Gateway 网关发送（对 WhatsApp 是幂等的）。
- `sudoclaw agent --message "hi" --to <num>` — 运行智能体轮次（默认等待最终结果）。
- `sudoclaw gateway call <method> --params '{"k":"v"}'` — 用于调试的原始方法调用器。
- `sudoclaw gateway stop|restart` — 停止/重启受监管的 Gateway 网关服务（launchd/systemd）。
- Gateway 网关辅助子命令假设 `--url` 上有运行中的 Gateway 网关；它们不再自动生成一个。

## 迁移指南

- 淘汰 `sudoclaw gateway` 和旧版 TCP 控制端口的使用。
