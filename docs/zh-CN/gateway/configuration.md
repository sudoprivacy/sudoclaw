...
read_when:
  - 添加或修改配置字段时
summary: ~/.sudoprivacy/sudoclaw.json 的所有配置选项及示例
title: 配置
x-i18n:
...
SudoClaw 从 `~/.sudoprivacy/sudoclaw.json` 读取可选的 **JSON5** 配置（支持注释和尾逗号）。

如果文件不存在，SudoClaw 使用安全的默认值（内置 Pi 智能体 + 按发送者分会话 + 工作区 `~/.openclaw/workspace`）。通常只在以下情况需要配置：
...
请使用 `config.patch` 或 `openclaw config set`。请备份 `~/.sudoprivacy/sudoclaw.json`。
...
- 默认日志文件：`/tmp/sudoprivacy/sudoclaw-YYYY-MM-DD.log`
- 如需稳定路径，将 `logging.file` 设为 `/tmp/sudoprivacy/sudoclaw.log`。
- 控制台输出可通过以下方式单独调整：
...
`~/.openclaw/agents/<agentId>/agent/models.json` 或在 SudoClaw 配置中的 `models.providers` 下定义相同的 schema 来添加自定义提供商（LiteLLM、本地 OpenAI 兼容服务器、Anthropic 代理等）。
...
- 配置文件颜色：`#FF4500`（龙虾橙）
- 注意：控制服务器由运行中的 Gateway 网关（SudoClaw.app 菜单栏或 `openclaw gateway`）启动。
...
- SudoClaw.app 监视 `~/.sudoprivacy/sudoclaw.json`，当 `gateway.mode` 或 `gateway.remote.url` 变更时实时切换模式。
...
Gateway 网关监视 `~/.sudoprivacy/sudoclaw.json`（或 `OPENCLAW_CONFIG_PATH`）并自动应用更改。
...
- `~/.sudoprivacy/sudoclaw.json`（或 `OPENCLAW_CONFIG_PATH`）
