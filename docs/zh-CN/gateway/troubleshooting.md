...
- `openclaw gateway status` 和 `sudoclaw doctor` 在服务看起来正在运行但端口关闭时会显示日志中的**最后 Gateway 网关错误**。
...
- 修复：从你希望服务使用的相同 `--profile` / `SUDOCLAW_STATE_DIR` 重新运行 `openclaw gateway install --force`。

**如果 `openclaw gateway status` 报告服务配置问题**

- 监管程序配置（launchd/systemd/schtasks）缺少当前默认值。
- 修复：运行 `sudoclaw doctor` 更新它（或 `openclaw gateway install --force` 完全重写）。
...
trash "${SUDOCLAW_STATE_DIR:-$HOME/.openclaw}/credentials" # 如果 logout 无法完全清除所有内容
...
trash "${SUDOCLAW_STATE_DIR:-$HOME/.openclaw}"
