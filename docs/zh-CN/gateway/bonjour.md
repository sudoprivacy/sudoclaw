...
SudoClaw 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
2. 在专用区域下发布 `_openclaw-gw._tcp` 的 DNS‑SD 记录
   （示例：`sudoclaw.internal.`）。
3. 配置 Tailscale **分割 DNS**，使你选择的域名通过该
   DNS 服务器为客户端（包括 iOS）解析。

SudoClaw 支持任何发现域名；`sudoclaw.internal.` 只是一个示例。
...
- 从 `~/.sudoclaw/dns/<domain>.db` 提供你选择的域名服务（示例：`sudoclaw.internal.`）
...
- 在 `~/.sudoclaw/sudoclaw.json` 中设置 `gateway.bind: "tailnet"`。
...
- `SUDOCLAW_DISABLE_BONJOUR=1` 禁用广播（旧版：`SUDOCLAW_DISABLE_BONJOUR`）。
- `~/.sudoclaw/sudoclaw.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `SUDOCLAW_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（旧版：`SUDOCLAW_SSH_PORT`）。
- `SUDOCLAW_TAILNET_DNS` 在 TXT 中发布 MagicDNS 提示（旧版：`SUDOCLAW_TAILNET_DNS`）。
- `SUDOCLAW_CLI_PATH` 覆盖广播的 CLI 路径（旧版：`SUDOCLAW_CLI_PATH`）。
