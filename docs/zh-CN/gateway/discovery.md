...
设计目标是将所有网络发现/广播保留在 **Node Gateway 网关**（`sudoclaw gateway`）中，并让客户端（mac 应用、iOS）作为消费者。
...
- `SUDOCLAW_DISABLE_BONJOUR=1` 禁用广播。
- `~/.sudoclaw/sudoclaw.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `SUDOCLAW_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（默认为 22）。
- `SUDOCLAW_TAILNET_DNS` 发布 `tailnetDns` 提示（MagicDNS）。
- `SUDOCLAW_CLI_PATH` 覆盖广播的 CLI 路径。
