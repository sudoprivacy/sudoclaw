...
通过一键模板在 Railway 上部署 SudoClaw，并在浏览器中完成设置。
...
- `SUDOCLAW_STATE_DIR=/data/.sudoclaw`（推荐）
- `SUDOCLAW_WORKSPACE_DIR=/data/workspace`（推荐）
- `SUDOCLAW_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 SudoClaw 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
