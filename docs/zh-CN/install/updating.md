...
read_when:
  - 更新 SudoClaw
  - 更新后出现问题
summary: 安全更新 SudoClaw（全局安装或源码），以及回滚策略
title: 更新
x-i18n:
...
SudoClaw 发展迅速（尚未到"1.0"）。将更新视为发布基础设施：更新 → 运行检查 → 重启（或使用会重启的 `sudoclaw update`）→ 验证。
...
- 快照你的定制内容：
  ...
  - 凭证：`~/.sudoclaw/credentials/`
  - 工作区：`~/.sudoclaw/workspace`
...
- 如果你的 Gateway 网关作为服务运行，`sudoclaw gateway restart` 优于杀死 PID。
...
如果你通过 **npm/pnpm** 安装（没有 git 元数据），`sudoclaw update` 将尝试通过你的包管理器更新。如果无法检测到安装，请改用"更新（全局安装）"。

## 更新（控制 UI / RPC）

控制 UI 有**更新并重启**（RPC：`update.run`）。它：

1. 运行与 `sudoclaw update` 相同的源码更新流程（仅限 git checkout）。
...
- 当你运行打包的 `openclaw` 二进制文件（[`openclaw.mjs`](https://github.com/sudoprivacy/sudoclaw/blob/main/openclaw.mjs)）或使用 Node 运行 `dist/` 时，`pnpm build` 很重要。
- 如果你从仓库 checkout 运行而没有全局安装，CLI 命令使用 `pnpm sudoclaw ...`。
- 如果你直接从 TypeScript 运行（`pnpm sudoclaw ...`），通常不需要重新构建，但**配置迁移仍然适用** → 运行 doctor。
...
注意：如果你是**源码安装**（git checkout），`sudoclaw doctor` 会提供先运行 `sudoclaw update`。
...
- 检测并将旧版 Gateway 网关服务（launchd/systemd；旧版 schtasks）迁移到当前 SudoClaw 服务。
...
- Linux systemd 用户服务：`systemctl --user restart sudoclaw-gateway[-<profile>].service`
- Windows（WSL2）：`systemctl --user restart sudoclaw-gateway[-<profile>].service`
  - `launchctl`/`systemctl` 仅在服务已安装时有效；否则运行 `sudoclaw gateway install`。
