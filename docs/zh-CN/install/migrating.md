...
- 复制**状态目录**（`$SUDOCLAW_STATE_DIR`，默认：`~/.sudoclaw/`）— 这包括配置、认证、会话和渠道状态。
- 复制你的**工作区**（默认 `~/.sudoclaw/workspace/`）— 这包括你的智能体文件（记忆、提示等）。
...
- `--profile <name>`（通常变成 `~/.sudoclaw-<profile>/`）
...
如果你有多个配置文件/状态目录（例如 `~/.sudoclaw-main`、`~/.sudoclaw-work`），分别归档每个。
...
在这个阶段，如果新手引导创建了一个新的 `~/.sudoclaw/` 也没关系 — 你将在下一步覆盖它。

### 步骤 2 — 将状态目录 + 工作区复制到新机器

复制**两者**：

- `$SUDOCLAW_STATE_DIR`（默认 `~/.sudoclaw/`）
- 你的工作区（默认 `~/.sudoclaw/workspace/`）
...
### 陷阱：只复制 `sudoclaw.json`

`sudoclaw.json` 是不够的。许多提供商在以下位置存储状态：

- `$SUDOCLAW_STATE_DIR/credentials/`
- `$SUDOCLAW_STATE_DIR/agents/<agentId>/...`

始终迁移整个 `$SUDOCLAW_STATE_DIR` 文件夹。
...
`$SUDOCLAW_STATE_DIR` 包含密钥（API 密钥、OAuth 令牌、WhatsApp 凭证）。将备份视为生产密钥：
...
- `sudoclaw status` 显示 Gateway 网关正在运行
