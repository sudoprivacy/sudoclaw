...
OpenClaw 从 `SUDOCLAW_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `SUDOCLAW_STATE_DIR` 中。

- `SUDOCLAW_STATE_DIR`（默认：`~/.openclaw`）
- `SUDOCLAW_CONFIG_PATH`（默认：`$OPENCLAW_STATE_DIR/openclaw.json`）
