...
使用 Nix 运行 SudoClaw 的推荐方式是通过 **[nix-openclaw](https://github.com/openclaw/nix-openclaw)** — 一个开箱即用的 Home Manager 模块。
...
当设置 `SUDOCLAW_NIX_MODE=1` 时（nix-openclaw 会自动设置）：
...
SudoClaw 从 `SUDOCLAW_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `SUDOCLAW_STATE_DIR` 中。

- `SUDOCLAW_STATE_DIR`（默认：`~/.sudoclaw`）
- `SUDOCLAW_CONFIG_PATH`（默认：`$SUDOCLAW_STATE_DIR/sudoclaw.json`）
