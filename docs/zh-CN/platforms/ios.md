...
- 网络路径：
  ...
  - 通过单播 DNS-SD 的 Tailnet（示例域：`sudoclaw.internal.`），**或**
  - 手动主机/端口（备选）。
...
如果 mDNS 被阻止，使用单播 DNS-SD 区域（选择一个域；示例：`sudoclaw.internal.`）和 Tailscale 分割 DNS。
...
```bash
sudoclaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18793/__openclaw__/canvas/"}'
```
...
- 配对提示从未出现：运行 `sudoclaw nodes pending` 并手动批准。
