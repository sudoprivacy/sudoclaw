...
- 在 Gateway 网关主机的 `~/.sudoclaw/sudoclaw.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway 网关主机上设置 DNS-SD 区域（示例 `sudoclaw.internal.`）并发布 `_openclaw-gw._tcp` 记录。
...
1. 在 Gateway 网关主机上创建 `~/.sudoclaw/workspace/canvas/index.html`。

2. 将节点导航到它（局域网）：

```bash
sudoclaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__openclaw__/canvas/"}'
```
