...
- 通过 Teams 私信、群聊或频道与 SudoClaw 交流。
...
- 启动时，SudoClaw 将团队/频道和用户允许列表名称解析为 ID（当 Graph 权限允许时）
  并记录映射；未解析的条目保持原样。
...
在配置 SudoClaw 之前，你需要创建一个 Azure Bot 资源。
...
1. **安装 Microsoft Teams 插件**
   - 从 npm：`sudoclaw plugins install @openclaw/msteams`
   - 从本地检出：`sudoclaw plugins install ./extensions/msteams`
...
SudoClaw 通过快速返回并主动发送回复来处理这个问题，但非常慢的响应仍可能导致问题。
...
默认情况下，SudoClaw 仅从 Microsoft/Teams 主机名下载媒体。使用 `channels.msteams.mediaAllowHosts` 覆盖（使用 `["*"]` 允许任何主机）。
...
上传的文件存储在配置的 SharePoint 站点默认文档库中的 `/SudoClawShared/` 文件夹中。

## 投票（Adaptive Cards）

SudoClaw 将 Teams 投票作为 Adaptive Cards 发送（没有原生 Teams 投票 API）。

- CLI：`sudoclaw message poll --channel msteams --target conversation:<id> ...`
- 投票由 Gateway 网关记录在 `~/.sudoclaw/msteams-polls.json` 中。
