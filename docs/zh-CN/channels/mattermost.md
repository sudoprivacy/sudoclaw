...
read_when:
  - 设置 Mattermost
  - 调试 Mattermost 路由
summary: Mattermost 机器人设置和 SudoClaw 配置
title: Mattermost
x-i18n:
...
```bash
sudoclaw plugins install @openclaw/mattermost
```

本地检出（从 git 仓库运行时）：

```bash
sudoclaw plugins install ./extensions/mattermost
```

如果你在配置/新手引导期间选择 Mattermost 并检测到 git 检出，SudoClaw 会自动提供本地安装路径。
...
- 通过以下方式批准：
  - `sudoclaw pairing list mattermost`
  - `sudoclaw pairing approve mattermost <CODE>`
...
在 `sudoclaw message send` 或 cron/webhooks 中使用这些目标格式：
