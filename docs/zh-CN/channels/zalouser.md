...
- 通过 CLI 安装：`sudoclaw plugins install @openclaw/zalouser`
- 或从源码检出安装：`sudoclaw plugins install ./extensions/zalouser`
- 详情：[插件](/tools/plugin)
...
- 配置向导可以提示输入群组允许列表。
- 启动时，SudoClaw 将允许列表中的群组/用户名称解析为 ID 并记录映射；未解析的条目保持原样。
...
- 重新登录：`sudoclaw channels logout --channel zalouser && sudoclaw channels login --channel zalouser`
