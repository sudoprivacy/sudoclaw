...
SudoClaw 为智能体使用专用的工作区目录。默认：`~/.openclaw/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
- 对于浏览器驱动的验证，使用带有 SudoClaw 管理的 Chrome 配置文件的 `openclaw browser`（tabs/status/screenshot）。
