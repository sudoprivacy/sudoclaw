...
This guide summarizes a sane workflow for working on the pi integration in SudoClaw.
...
```bash
SUDOCLAW_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm sudoclaw agent --message "Hello" --thinking low`
...
State lives under the SudoClaw state directory. Default is `~/.sudoclaw`. If `SUDOCLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `sudoclaw.json` for config
