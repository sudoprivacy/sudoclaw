...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/sudoclaw`)
...
you revoke it with `sudoclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.sudoprivacy/sudoclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop sudoclaw`, `please stop`) to abort out-of-band
...
```bash
SUDOCLAW_CONTROL_UI_BASE_PATH=/sudoclaw/ pnpm ui:build
```
