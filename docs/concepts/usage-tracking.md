...
- `/usage cost` in chats: local cost summary aggregated from SudoClaw session logs.
- CLI: `sudoclaw status --usage` prints a full per-provider breakdown.
- CLI: `sudoclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
