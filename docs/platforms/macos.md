...
- Installs the global CLI (`sudoclaw`) via npm/pnpm on request (bun not recommended for the Gateway runtime).
...
(or `ai.sudoclaw.<profile>` when using `--profile`/`SUDOCLAW_PROFILE`; legacy `com.sudoclaw.*` still unloads).
...
Replace the label with `ai.sudoclaw.<profile>` when running a named profile.
...
The app registers the `sudoclaw://` URL scheme for local actions.
...
```bash
open 'sudoclaw://agent?message=Hello%20from%20deep%20link'
```
...
```bash
cd apps/macos
swift run sudoclaw-mac connect --json
swift run sudoclaw-mac discover --timeout 3000 --json
```
