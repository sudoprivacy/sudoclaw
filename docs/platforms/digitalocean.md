---
summary: "SudoClaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up SudoClaw on DigitalOcean
  - Looking for cheap VPS hosting for SudoClaw
title: "DigitalOcean"
---

# SudoClaw on DigitalOcean

## Goal

Run a persistent SudoClaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status sudoclaw-gateway.service

# View logs
journalctl --user -u sudoclaw-gateway.service -f
```
...
```bash
sudoclaw channels login whatsapp
# Scan QR code
```
...
- `~/.sudoclaw/workspace/` — workspace (SOUL.md, memory, etc.)

These survive reboots. Back them up periodically:

```bash
tar -czvf sudoclaw-backup.tar.gz ~/.sudoclaw ~/.sudoclaw/workspace
```
...
journalctl -u sudoclaw --no-pager -n 50
```
