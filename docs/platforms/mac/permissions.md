...
- Same path: run the app from a fixed location (for SudoClaw, `dist/SudoClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.sudoclaw.mac
sudo tccutil reset ScreenCapture ai.sudoclaw.mac
...
Workaround: move files into the SudoClaw workspace (`~/.sudoclaw/workspace`) if you want to avoid per-folder grants.
