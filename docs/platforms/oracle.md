...
```bash
# Set hostname
sudo hostnamectl set-hostname sudoclaw
...
sudo tailscale up --ssh --hostname=sudoclaw
```

This enables Tailscale SSH, so you can connect via `ssh sudoclaw` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@sudoclaw` (or use the Tailscale IP).
...
```
https://sudoclaw.<tailnet-name>.ts.net/
```
...
ssh -L 18789:127.0.0.1:18789 ubuntu@sudoclaw
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=sudoclaw --reset
```
