...
description: "Complete field-by-field reference for ~/.sudoprivacy/sudoclaw.json"
summary: "Complete reference for every SudoClaw config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.sudoprivacy/sudoclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — SudoClaw uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `sudoclaw doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["sudoclaw-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-sudoclaw",
          requireMention: false,
...
- SudoClaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `sudoclaw plugins install @sudoclaw/mattermost`.
...
SudoClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `sudoclaw channels add` (or channel onboarding) while still on a single-account top-level channel config, SudoClaw moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `sudoclaw doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@sudoclaw", "sudoclaw"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@sudoclaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `sudoclaw.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, SudoClaw auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/sudoclaw" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, SudoClaw assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.sudoclaw/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.sudoclaw/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@sudoclaw"] },
        sandbox: { mode: "off" },
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, SudoClaw starts a fresh thread session instead of inheriting parent transcript history.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:sudoclaw`   | All built-in tools (excludes provider plugins)                                           |
...
SudoClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.sudoclaw/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `SUDOCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `sudoclaw onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `sudoclaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `sudoclaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `sudoclaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `sudoclaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `sudoclaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.sudoclaw/extensions`, `<workspace>/.sudoclaw/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `sudoclaw plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or SUDOCLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `SUDOCLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.sudoclaw-dev` + port `19001`), `--profile <name>` (uses `~/.sudoclaw-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-sudoclaw-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `SUDOCLAW_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or SUDOCLAW_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__sudoclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__sudoclaw__/a2ui/`
...
- Also serves A2UI at `/__sudoclaw__/a2ui/`.
...
- Hostname defaults to `sudoclaw`. Override with `SUDOCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.sudoclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `sudoclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.sudoclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${SUDOCLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.sudoclaw/credentials/oauth.json`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/sudoprivacy/sudoclaw.log",
    consoleLevel: "info",
...
- Default log file: `/tmp/sudoprivacy/sudoclaw-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `sudoclaw doctor --fix` can strip unknown keys).
