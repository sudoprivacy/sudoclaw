...
description: "Complete field-by-field reference for ~/.sudoclaw/sudoclaw.json"
...
Every field available in `~/.sudoclaw/sudoclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).
...
Mattermost ships as a plugin: `sudoclaw plugins install @openclaw/mattermost`.
...
- Hostname defaults to `openclaw`. Override with `SUDOCLAW_MDNS_HOSTNAME`.
...
- Default log file: `/tmp/sudoclaw/openclaw-YYYY-MM-DD.log`.
