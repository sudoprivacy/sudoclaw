...
- Default rolling log file is under `/tmp/sudoclaw/` (one file per day): `sudoclaw-YYYY-MM-DD.log`
  - Date uses the gateway host's local timezone.
- The log file path and level can be configured via `~/.sudoprivacy/sudoclaw.json`:
  - `logging.file`
  - `logging.level`
...
`sudoclaw gateway` supports a per-gateway style switch:
