...
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                          | filesystem perms on `~/.sudoclaw/sudoclaw.json`                                                   | yes      |
...
- `~/.sudoclaw/sudoclaw.json`: `600` (user read/write only)
...
1. Check Gateway logs: `/tmp/sudoclaw/openclaw-YYYY-MM-DD.log` (or `logging.file`).
