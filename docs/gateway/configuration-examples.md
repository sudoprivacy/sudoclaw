...
Save to `~/.sudoprivacy/sudoclaw.json` and you can DM the bot from that number.
...
```json5
{
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/sudoclaw" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
