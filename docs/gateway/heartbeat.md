...
- During heartbeat runs, SudoClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `sudoclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, SudoClaw skips the heartbeat run entirely (no model call).
...
```bash
sudoclaw system event --text "Check for urgent follow-ups" --mode now
```
