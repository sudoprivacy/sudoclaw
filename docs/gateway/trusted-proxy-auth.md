...
read_when:
  - Running SudoClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of SudoClaw
...
- You run SudoClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. SudoClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. SudoClaw extracts the user identity from the configured header
...
- SudoClaw can stay on loopback HTTP behind the proxy.
...
If SudoClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://sudoclaw.example.com
    to: http://sudoclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
sudoclaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy sudoclaw:18789 {
...
location / {
    ...
    proxy_pass http://sudoclaw:18789;
...
3. Update SudoClaw config with trusted-proxy auth
