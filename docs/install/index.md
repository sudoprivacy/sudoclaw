    ...
    <Tabs>
      <Tab title="npm">
        ...
        <Accordion title="sharp build errors?">
          ...
          ```bash
          SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g sudoclaw@latest
          ```
      ...
      <Tab title="pnpm">
        ...
        pnpm approve-builds -g        # approve sudoclaw, node-llama-cpp, sharp, etc.
        sudoclaw onboard --install-daemon
        ```
  ...
  <Accordion title="From source" icon="github">
    ...
    <Steps>
      ...
      <Step title="Link the CLI">
        Make the `sudoclaw` command available globally:
...
## Troubleshooting: `sudoclaw` not found
...
If `$(npm prefix -g)/bin` (macOS/Linux) or `$(npm prefix -g)` (Windows) is **not** in your `$PATH`, your shell can't find global npm binaries (including `sudoclaw`).
