...
Deploy SudoClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/sudoprivacy/sudoclaw)
...
repository configures everything needed to run SudoClaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SUDOCLAW_STATE_DIR
        value: /data/.sudoclaw
      - key: SUDOCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: SUDOCLAW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/sudoclaw`.
...
If you use the original SudoClaw repository, Render will not auto-deploy your SudoClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For SudoClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any SudoClaw host.
