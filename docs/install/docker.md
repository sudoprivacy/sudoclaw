...
- [GitHub Container Registry package](https://github.com/sudoprivacy/sudoclaw/pkgs/container/sudoclaw)
...
```bash
export SUDOCLAW_HOME_VOLUME="sudoclaw_home"
./docker-setup.sh
```
...
```bash
sudo chown -R 1000:1000 /path/to/sudoclaw-config /path/to/sudoclaw-workspace
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-sudoclaw-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-sudoclaw-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-sudoclaw-sbx" } },
    },
  },
}
```
