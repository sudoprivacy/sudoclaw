...
read_when:
  - You want to understand `sudoclaw.ai/install.sh`
...
SudoClaw ships three installer scripts, served from `sudoclaw.ai`.
...
<Tabs>
  <Tab title="install.sh">
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://sudoclaw.ai/install.sh | bash -s -- --help
    ```

  </Tab>
  <Tab title="install-cli.sh">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://sudoclaw.ai/install-cli.sh | bash
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://sudoclaw.ai/install-cli.sh | bash -s -- --help
    ```
  ...
  <Tab title="install.ps1">
    ...
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://sudoclaw.ai/install.ps1))) -Tag beta -NoOnboard -DryRun
    ```
...
<Note>
If install succeeds but `sudoclaw` is not found in a new terminal, see [Node.js troubleshooting](/install/node#troubleshooting).
</Note>
...
<Steps>
  ...
  <Step title="Install SudoClaw">
    ...
    - `git` method: clone/update repo, install deps with pnpm, build, then install wrapper at `~/.local/bin/sudoclaw`
  </Step>
  <Step title="Post-install tasks">
    - Runs `sudoclaw doctor --non-interactive` on upgrades and git installs (best effort)
...
<Tabs>
  ...
  <Tab title="Skip onboarding">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://sudoclaw.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  ...
  <Tab title="Dry run">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://sudoclaw.ai/install.sh | bash -s -- --dry-run
    ```
  </Tab>
</Tabs>
...
| `--git-dir <path>`              | Checkout directory (default: `~/sudoclaw`). Alias: `--dir` |
...
<Steps>
  ...
  <Step title="Install SudoClaw under prefix">
    Installs with npm using `--prefix <prefix>`, then writes wrapper to `<prefix>/bin/sudoclaw`.
  </Step>
</Steps>
...
<Tabs>
  ...
  <Tab title="Automation JSON output">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://sudoclaw.ai/install-cli.sh | bash -s -- --json --prefix /opt/sudoclaw
    ```
  </Tab>
  <Tab title="Run onboarding">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://sudoclaw.ai/install-cli.sh | bash -s -- --onboard
    ```
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Install SudoClaw">
    ...
    - `git` method: clone/update repo, install/build with pnpm, and install wrapper at `%USERPROFILE%\.local\bin\sudoclaw.cmd`
  </Step>
  <Step title="Post-install tasks">
    Adds needed bin directory to user PATH when possible, then runs `sudoclaw doctor --non-interactive` on upgrades and git installs (best effort).
  </Step>
</Steps>
...
<Tabs>
  ...
  <Tab title="Git install">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://sudoclaw.ai/install.ps1))) -InstallMethod git
    ```
  </Tab>
  <Tab title="Custom git directory">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://sudoclaw.ai/install.ps1))) -InstallMethod git -GitDir "C:\sudoclaw"
    ```
  </Tab>
  <Tab title="Dry run">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://sudoclaw.ai/install.ps1))) -DryRun
    ```
  </Tab>
...
| `-GitDir <path>`          | Checkout directory (default: `%USERPROFILE%\sudoclaw`) |
...
<Tabs>
  <Tab title="install.sh (non-interactive npm)">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://sudoclaw.ai/install.sh | bash -s -- --no-prompt --no-onboard
    ```
  </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="sharp/libvips issues">
    ...
    ```bash
    SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL --proto '=https' --tlsv1.2 https://sudoclaw.ai/install.sh | bash
    ```
  ...
  <Accordion title='Windows: "sudoclaw is not recognized"'>
  ...
  <Accordion title="sudoclaw not found after install">
