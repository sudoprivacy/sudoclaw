...
| `internal_port = 3000`         | Must match `--port 3000` (or `SUDOCLAW_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
| `SUDOCLAW_STATE_DIR = "/data"` | Persists state on the volume                                                |

## 3) Set secrets

```bash
# Required: Gateway token (for non-loopback binding)
fly secrets set SUDOCLAW_GATEWAY_TOKEN=$(openssl rand -hex 32)
...
Or visit `https://my-sudoclaw.fly.dev/`

Paste your gateway token (the one from `SUDOCLAW_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `SUDOCLAW_GATEWAY_PORT=3000`).
...
**Fix:** Ensure `SUDOCLAW_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-sudoclaw

# Release public IPs
fly ips release <public-ipv4> -a my-sudoclaw
fly ips release <public-ipv6> -a my-sudoclaw
...
fly ips allocate-v6 --private -a my-sudoclaw
```
...
fly proxy 3000:3000 -a my-sudoclaw
