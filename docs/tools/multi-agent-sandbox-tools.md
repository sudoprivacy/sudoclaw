...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `sudoclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `sudoclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=sudoclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${SUDOCLAW_STATE_DIR:-$HOME/.sudoclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
