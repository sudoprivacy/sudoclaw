---
title: Lobster
summary: "Typed workflow runtime for SudoClaw with resumable approval gates."
description: Typed workflow runtime for SudoClaw — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets SudoClaw run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: SudoClaw runs one Lobster tool call and gets a structured result.
...
SudoClaw launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | sudoclaw.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
sudoclaw.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In SudoClaw tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the SudoClaw Gateway (see the [Lobster repo](https://github.com/sudoprivacy/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), SudoClaw keeps core tools enabled. To restrict core
...
→ sudoclaw calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ sudoclaw calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls SudoClaw tools that do.
