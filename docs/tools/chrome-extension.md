---
summary: "Chrome extension: let SudoClaw drive your existing Chrome tab"
read_when:
...
The SudoClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate sudoclaw-managed Chrome profile.
...
SudoClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the SudoClaw release (npm package) as static files. There is no separate “build” step.

After upgrading SudoClaw:

- Re-run `sudoclaw browser extension install` to refresh the installed files under your SudoClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

SudoClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `SUDOCLAW_GATEWAY_TOKEN`)
...
- Open the tab you want SudoClaw to control.
...
- `ON`: attached; SudoClaw can drive that tab.
...
Debugging: `sudoclaw sandbox explain`
...
`sudoclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `sudoclaw browser extension install` first to copy the extension to a stable location under your SudoClaw state directory.
...
- **This is not isolated** like the dedicated sudoclaw-managed profile.
