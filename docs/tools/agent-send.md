---
summary: "Direct `sudoclaw agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `sudoclaw agent` (direct agent runs)

`sudoclaw agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `sudoclaw message --target`).
...
```bash
sudoclaw agent --to +15555550123 --message "status update"
...
sudoclaw agent --to +15555550123 --message "Summon reply" --deliver
