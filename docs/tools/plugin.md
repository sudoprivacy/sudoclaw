...
```bash
sudoclaw plugins install @sudoclaw/voice-call
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@sudoclaw/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@sudoclaw/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@sudoclaw/zalouser`
- [Matrix](/channels/matrix) — `@sudoclaw/matrix`
- [Nostr](/channels/nostr) — `@sudoclaw/nostr`
- [Zalo](/channels/zalo) — `@sudoclaw/zalo`
- [Microsoft Teams](/channels/msteams) — `@sudoclaw/msteams`
...
A plugin directory may include a `package.json` with `sudoclaw.extensions`:
...
Security guardrail: every `sudoclaw.extensions` entry must stay inside the plugin
...
Channel plugins can advertise onboarding metadata via `sudoclaw.channel` and
install hints via `sudoclaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@sudoclaw/nextcloud-talk",
  "sudoclaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@sudoclaw/nextcloud-talk",
...
sudoclaw plugins install @sudoclaw/voice-call # install from npm
sudoclaw plugins install @sudoclaw/voice-call --pin # store exact resolved name@version
...
- Main package: `sudoclaw` (this repo)
- Plugins: separate npm packages under `@sudoclaw/*` (example: `@sudoclaw/voice-call`)

Publishing contract:

- Plugin `package.json` must include `sudoclaw.extensions` with one or more entry files.
...
- Separately published plugins should run their own CI (lint/build/test) and validate `sudoclaw.extensions` points at the built entrypoint (`dist/index.js`).
