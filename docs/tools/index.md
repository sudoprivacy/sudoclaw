---
summary: "Agent tool surface for SudoClaw (browser, canvas, nodes, message, cron) replacing legacy `sudoclaw-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `sudoclaw-*` skills
title: "Tools"
---

# Tools (SudoClaw)

SudoClaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `sudoclaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `sudoclaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, SudoClaw logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:sudoclaw`: all built-in SudoClaw tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`sudoclaw node run`).
...
SudoClaw tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `sudoclaw configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated SudoClaw-managed browser.
...
- Quick smoke: `sudoclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `sudoclaw gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, SudoClaw uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, SudoClaw clamps `tools.sessions.visibility` to `tree`.
