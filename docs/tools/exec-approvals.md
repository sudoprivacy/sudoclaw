...
- **gateway host** → `sudoclaw` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.sudoclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.sudoclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `sudoclaw approvals allowlist ...`).
- `sudoclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `sudoclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.sudoclaw/exec-approvals.json` directly.

CLI: `sudoclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
