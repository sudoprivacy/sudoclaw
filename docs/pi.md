...
summary: "Architecture of SudoClaw's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in SudoClaw
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how SudoClaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

SudoClaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, SudoClaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in SudoClaw's local TUI mode)                                             |
...
├── pi-tools.ts                    # createSudoClawCodingTools()
...
├── openclaw-tools.ts              # SudoClaw-specific tools
...
Image injection is prompt-local: SudoClaw loads image refs from the current prompt and
...
2. **Custom Replacements**: SudoClaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **SudoClaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
SudoClaw wraps this with `guardSessionManager()` for tool result safety.
...
SudoClaw maintains an auth profile store with multiple API keys per provider:
...
SudoClaw loads custom pi extensions for specialized behavior:
...
SudoClaw also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | SudoClaw Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom SudoClaw tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.sudoclaw/agents/<agentId>/sessions/` (or `$SUDOCLAW_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `SUDOCLAW_LIVE_TEST=1`)
