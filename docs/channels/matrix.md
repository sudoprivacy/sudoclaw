...
Matrix is an open, decentralized messaging protocol. SudoClaw connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `sudoclaw plugins install @openclaw/matrix`
   - From a local checkout: `sudoclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: SudoClaw calls the same
     login endpoint, stores the access token in `~/.sudoclaw/credentials/matrix/credentials.json`,
...
- On first connection, SudoClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  SudoClaw logs a warning.
...
`~/.sudoclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, SudoClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
