...
Tlon is a decentralized messenger built on Urbit. SudoClaw connects to your Urbit ship and can
...
```bash
sudoclaw plugins install @sudoclaw/tlon
```

Local checkout (when running from a git repo):

```bash
sudoclaw plugins install ./extensions/tlon
```
...
By default, SudoClaw blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `sudoclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, SudoClaw replies in-thread.
