...
read_when:
  - Setting up Twitch chat integration for SudoClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. SudoClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
sudoclaw plugins install @openclaw/twitch
```
...
4. Configure the token:
   - Env: `SUDOCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "sudoclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SUDOCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
SUDOCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
