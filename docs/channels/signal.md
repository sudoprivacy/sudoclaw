...
- SudoClaw installed on your server (Linux flow below tested on Ubuntu 24).
...
4. Configure SudoClaw and restart the gateway.
5. Send a first DM and approve pairing (`sudoclaw pairing approve signal <CODE>`).
...
4. Configure SudoClaw, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `sudoclaw pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point SudoClaw at it:
...
This skips auto-spawn and the startup wait inside SudoClaw. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `sudoclaw pairing list signal`
  - `sudoclaw pairing approve signal <CODE>`
...
- **Typing indicators**: SudoClaw sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, SudoClaw forwards read receipts for allowed DMs.
...
sudoclaw pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/sudoprivacy/sudoclaw-$(date +%Y-%m-%d).log" | tail -20
```
