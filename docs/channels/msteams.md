...
```bash
sudoclaw plugins install @sudoclaw/msteams
```

Local checkout (when running from a git repo):

```bash
sudoclaw plugins install ./extensions/msteams
```
...
3. Configure SudoClaw with those credentials.
...
- Talk to SudoClaw via Teams DMs, group chats, or channels.
...
- On startup, SudoClaw resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.sudoprivacy/sudoclaw.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring SudoClaw, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `sudoclaw-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `sudoclaw plugins install @sudoclaw/msteams`
   - From a local checkout: `sudoclaw plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "SudoClaw in Teams", "full": "SudoClaw in Teams" },
...
SudoClaw handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, SudoClaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/SudoClawShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

SudoClaw sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `sudoclaw message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.sudoclaw/msteams-polls.json`.
...
```bash
sudoclaw message send --channel msteams \
...
sudoclaw message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
sudoclaw message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
sudoclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
sudoclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
