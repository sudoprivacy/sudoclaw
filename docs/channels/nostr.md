...
read_when:
  - You want SudoClaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables SudoClaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`sudoclaw onboard`) and `sudoclaw channels add` list optional channel plugins.
...
```bash
sudoclaw plugins install --link <path-to-sudoclaw>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "sudoclaw@example.com",
        "lud16": "sudoclaw@example.com"
      }
    }
  }
}
