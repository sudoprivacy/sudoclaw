...
SudoClaw features that can generate provider usage or paid API calls.
...
- `sudoclaw status --usage` and `sudoclaw channels list` show provider **usage windows**
...
SudoClaw can pick up credentials from:
...
- `sudoclaw status --usage`
- `sudoclaw models status --json`
...
`sudoclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
