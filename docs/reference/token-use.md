---
summary: "How SudoClaw builds prompt context and reports token usage + costs"
read_when:
...
SudoClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

SudoClaw assembles its own system prompt on every run. It includes:
...
For images, SudoClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from SudoClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `sudoclaw status --usage` and `sudoclaw channels list` show
...
`cacheWrite`. If pricing is missing, SudoClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. SudoClaw can
...
Anthropic's 1M context window is currently beta-gated. SudoClaw can inject the
...
SudoClaw skips the `context-1m-*` beta header because Anthropic currently
