...
SudoClaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- SudoClaw owns lifecycle when `channels.signal.autoStart=true`.
...
- SudoClaw spawns `imsg rpc` as a child process (legacy iMessage integration).
