...
- With Anthropic API-key auth profiles, SudoClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, SudoClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
SudoClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.sudoclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$SUDOCLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `SUDOCLAW_CACHE_TRACE=1` enables cache tracing.
- `SUDOCLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `SUDOCLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `SUDOCLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `SUDOCLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
