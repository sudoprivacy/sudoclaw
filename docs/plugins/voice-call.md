...
read_when:
  - You want to place an outbound voice call from SudoClaw
...
Voice calls for SudoClaw via a plugin. Supports outbound notifications and
...
- Use `sudoclaw voicecall ...` or the `voice_call` tool
...
```bash
sudoclaw plugins install ./extensions/voice-call
