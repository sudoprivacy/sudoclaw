...
read_when:
  - You want Zalo Personal (unofficial) support in SudoClaw
...
Zalo Personal support for SudoClaw via a plugin, using `zca-cli` to automate a normal Zalo user account.
...
```bash
sudoclaw plugins install @openclaw/zalouser
```
...
```bash
sudoclaw plugins install ./extensions/zalouser
