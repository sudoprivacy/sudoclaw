---
summary: "Use Venice AI privacy-focused models in SudoClaw"
read_when:
  - You want privacy-focused inference in SudoClaw
...
```bash
sudoclaw onboard --auth-choice venice-api-key
```
...
```bash
sudoclaw agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, SudoClaw shows all available Venice models. Pick based on your needs:
...
```bash
sudoclaw models set venice/claude-opus-45
sudoclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
sudoclaw models list | grep venice
```

## Configure via `sudoclaw configure`

1. Run `sudoclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
SudoClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
sudoclaw agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
sudoclaw agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
sudoclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
sudoclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
sudoclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `sudoclaw models list` to see currently available models. Some models may be temporarily offline.
