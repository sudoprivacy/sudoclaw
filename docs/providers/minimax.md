---
summary: "Use MiniMax M2.1 in SudoClaw"
read_when:
  - You want MiniMax models in SudoClaw
...
```bash
sudoclaw plugins enable minimax-portal-auth  # skip if already loaded.
sudoclaw gateway restart  # restart if gateway is already running
sudoclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/sudoprivacy/sudoclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `sudoclaw.json`:
...
- Use `sudoclaw models list` and `sudoclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `sudoclaw configure` and selecting **MiniMax M2.1**, or
