...
function setupSessionState(cfg: SudoClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: SudoClawConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "sudoclaw-doctor-state-integrity-"));
  ...
  it("prints sudoclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: SudoClawConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
