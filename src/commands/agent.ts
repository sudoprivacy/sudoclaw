...
function prependInternalEventContext(
  ...
  if (body.includes("SudoClaw runtime context (internal):")) {
    return body;
  }
...
export async function agentCommand(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("sudoclaw agents list")}" to see configured agents.`,
      );
    }
  }
