...
export function normalizeCompatibilityConfigValues(cfg: SudoClawConfig): {
  config: SudoClawConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: SudoClawConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as SudoClawConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as SudoClawConfig["browser"],
    };
    changes.push(
