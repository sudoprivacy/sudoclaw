...
const SEARCH_TOOL = "https://docs.sudoclaw.ai/mcp.SearchOpenClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.sudoclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('sudoclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.sudoclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('sudoclaw docs "your query"')}`);
    }
    return;
  }
