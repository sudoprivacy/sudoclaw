...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under SUDOCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("sudoclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-oauth-sync-"));
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-oauth-nosync-"));
    process.env.SUDOCLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside SUDOCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-oauth-external-"));
    process.env.SUDOCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* SUDOCLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to SUDOCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("sudoclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
