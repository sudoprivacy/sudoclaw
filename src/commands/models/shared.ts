...
export async function loadValidConfigOrThrow(): Promise<SudoClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: SudoClawConfig) => SudoClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: SudoClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: SudoClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("sudoclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
