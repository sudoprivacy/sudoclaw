...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveSudoClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("sudoclaw models auth setup-token")}\` or \`${formatCliCommand("sudoclaw configure")}\`.`
          : `Run \`${formatCliCommand("sudoclaw configure")}\` or set an API key env var.`;
