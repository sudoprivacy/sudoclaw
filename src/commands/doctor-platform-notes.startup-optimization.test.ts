...
describe("noteStartupOptimizationHints", () => {
  ...
  it("warns when compile cache is under /tmp and no-respawn is not set", () => {
    ...
    noteStartupOptimizationHints(
      {
        NODE_COMPILE_CACHE: "/tmp/sudoclaw-compile-cache",
      },
      { platform: "linux", arch: "arm64", totalMemBytes: 4 * 1024 ** 3, noteFn },
    );
    ...
    expect(message).toContain("SUDOCLAW_NO_RESPAWN is not set to 1");
    expect(message).toContain("export NODE_COMPILE_CACHE=/var/tmp/sudoclaw-compile-cache");
    expect(message).toContain("export SUDOCLAW_NO_RESPAWN=1");
  });
