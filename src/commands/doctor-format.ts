...
export function buildGatewayRuntimeHints(
  ...
  if (runtime.cachedLabel && platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(env.SUDOCLAW_PROFILE);
    hints.push(
      `LaunchAgent label cached but plist missing. Clear with: launchctl bootout gui/$UID/${label}`,
    );
    hints.push(`Then reinstall: ${formatCliCommand("sudoclaw gateway install", env)}`);
  }
  ...
  if (runtime.status === "stopped") {
    ...
    if (platform === "darwin") {
      ...
      const task = resolveGatewayWindowsTaskName(env.SUDOCLAW_PROFILE);
