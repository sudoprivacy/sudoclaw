...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: SudoClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: SudoClawConfig) => { changed: boolean; next: SudoClawConfig };
  defaultModel: string;
  overrideConfig: SudoClawConfig;
  alreadyDefaultConfig: SudoClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: SudoClawConfig) => config,
      applyDefaultConfig: (config: SudoClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: SudoClawConfig = { agents: { defaults: {} } };
