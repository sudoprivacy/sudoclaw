  ...
  return await withProgress(
    ...
    async (progress) => {
      ...
      const channels = await buildChannelsTable(cfg, {
        ...
        // Set `SUDOCLAW_SHOW_SECRETS=0` (or legacy `OPENCLAW_SHOW_SECRETS=0` / `CLAWDBOT_SHOW_SECRETS=0`) to force redaction.
        showSecrets:
          (
            process.env.SUDOCLAW_SHOW_SECRETS ??
            process.env.OPENCLAW_SHOW_SECRETS ??
            process.env.CLAWDBOT_SHOW_SECRETS
          )?.trim() !== "0",
      });
      progress.tick();
