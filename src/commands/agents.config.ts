...
function resolveAgentName(cfg: SudoClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: SudoClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SudoClawConfig): AgentSummary[] {
