...
type PromptDefaultModelResult = { model?: string; config?: SudoClawConfig };
...
function resolveConfiguredModelRaw(cfg: SudoClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: SudoClawConfig): string[] {
...
export function applyPrimaryModel(cfg: SudoClawConfig, model: string): SudoClawConfig {
...
export function applyModelAllowlist(cfg: SudoClawConfig, models: string[]): SudoClawConfig {
  const defaults = cfg.agents?.defaults;
