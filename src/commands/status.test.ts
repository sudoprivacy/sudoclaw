...
beforeAll(() => {
  envSnapshot = captureEnv(["SUDOCLAW_PROFILE"]);
  process.env.SUDOCLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/sudoclaw-root.js", () => ({
  resolveSudoClawPackageRoot: vi.fn().mockResolvedValue("/tmp/sudoclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/sudoclaw/pnpm-lock.yaml",
      markerPath: "/tmp/sudoclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.sudoclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.sudoclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("SudoClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("sudoclaw status --all") ||
          l.includes("sudoclaw --profile isolated status --all") ||
          l.includes("sudoclaw status --all") ||
          l.includes("sudoclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.SUDOCLAW_GATEWAY_TOKEN = "abcd1234";
    try {
