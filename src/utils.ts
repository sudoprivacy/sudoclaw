...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".sudoclaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.SUDOCLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$SUDOCLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via SUDOCLAW_STATE_DIR.
