...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "sudoclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "sudoclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.SUDOCLAW_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.SUDOCLAW_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.SUDOCLAW_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[sudoclaw] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    import("./version.js")
      ...
      .catch((error) => {
        console.error(
          "[sudoclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[sudoclaw] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[sudoclaw] ${parsed.error}`);
      process.exit(2);
    }
