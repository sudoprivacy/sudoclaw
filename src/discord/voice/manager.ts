...
function resolveVoiceTtsConfig(params: { cfg: SudoClawConfig; override?: TtsConfig }): {
  cfg: SudoClawConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredSudoClawTmpDir(), "discord-voice-"));
