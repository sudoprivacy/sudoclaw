  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/sudoclaw",
      },
    },
    session: { store: "/tmp/sudoclaw-sessions.json" },
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bsudoclaw\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "sudoclaw: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "SudoClaw" },
          },
        },
        eventPatch: {
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/sudoclaw" },
      session: { store: "/tmp/sudoclaw-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
