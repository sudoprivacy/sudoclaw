...
function listAgents(cfg: SudoClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: SudoClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: SudoClawConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<SudoClawConfig, EvaluatedBindingsCache>();
