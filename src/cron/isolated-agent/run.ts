...
export type RunCronAgentTurnResult = {
   ...
   * messages.  See: https://github.com/sudoprivacy/sudoclaw/issues/15692
   */
  delivered?: boolean;
  /**
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.SUDOCLAW_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.sudoclaw/agents/<agentId>/agent/).
  ...
  const cfgWithAgentDefaults: SudoClawConfig = {
    ...params.cfg,
    ...
    const fallbackResult = await runWithModelFallback({
      ...
      run: (providerOverride, modelOverride) => {
        ...
        if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
          ...
          // the configured timeoutSeconds. See: https://github.com/sudoprivacy/sudoclaw/issues/29774
