...
function telegramCfg(): SudoClawConfig {
  return { channels: { telegram: { botToken: "tok" } } } as SudoClawConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: SudoClawConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as SudoClawConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as SudoClawConfig;
...
function createSignalAccountOverrideCfg(): SudoClawConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as SudoClawConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as SudoClawConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as SudoClawConfig,
        accountId: undefined,
        params: {
