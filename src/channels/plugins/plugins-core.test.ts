...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@sudoclaw/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "sudoclaw-catalog-"));
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: SudoClawConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
