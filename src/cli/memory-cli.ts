    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["sudoclaw memory status", "Show index and provider status."],
          ["sudoclaw memory index --force", "Force a full reindex."],
          ['sudoclaw memory search --query "deployment notes"', "Search indexed memory entries."],
          ["sudoclaw memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.sudoclaw.ai/cli/memory")}\n`,
    );
