...
import type { SudoClawConfig, ConfigFileSnapshot } from "../config/types.sudoclaw.js";
...
vi.mock("../infra/sudoclaw-root.js", () => ({
  resolveSudoClawPackageRoot: vi.fn(),
}));
...
const { resolveSudoClawPackageRoot } = await import("../infra/sudoclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/sudoclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as SudoClawConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/sudoclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveSudoClawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("sudoclaw-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveSudoClawPackageRoot).mockClear();
    ...
    vi.mocked(resolveSudoClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("sudoclaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/sudoclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "sudoclaw-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("SudoClaw update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as SudoClawConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveSudoClawPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("sudoclaw-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ SUDOCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.SUDOCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("sudoclaw-update-wizard");
    await withEnvAsync({ SUDOCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
