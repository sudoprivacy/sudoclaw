...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: SudoClawConfig; warnings: string[] } {
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.sudoclaw.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: SudoClawConfig = enableResult.config;
    ...
    .action(async (raw: string, opts: { link?: boolean; pin?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: SudoClawConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.sudoclaw.ai/plugin");
      lines.push("");
