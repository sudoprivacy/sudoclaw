...
import { resolveSudoClawPackageRoot } from "../../infra/sudoclaw-root.js";
...
const SUDOCLAW_REPO_URL = "https://github.com/sudoprivacy/sudoclaw.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "sudoclaw";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("sudoclaw@")) {
    return trimmed.slice("sudoclaw@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.SUDOCLAW_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveSudoClawPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", SUDOCLAW_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `SUDOCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set SUDOCLAW_GIT_DIR to an empty folder or an sudoclaw checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`SUDOCLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "sudoclaw.mjs");
