...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("uses SUDOCLAW_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        SUDOCLAW_PROFILE: "default",
        SUDOCLAW_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("uses SUDOCLAW_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        SUDOCLAW_PROFILE: "default",
        SUDOCLAW_LAUNCHD_LABEL: "com.custom.sudoclaw",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.sudoclaw'");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        SUDOCLAW_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.sudoclaw.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "SudoClaw Gateway (production)"');
      await cleanupScript(scriptPath);
    });
