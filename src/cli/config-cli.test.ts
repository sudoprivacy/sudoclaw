...
import type { ConfigFileSnapshot, SudoClawConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `sudoclaw config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: SudoClawConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: SudoClawConfig, options?: { unsetPaths?: string[][] }) =>
...
function buildSnapshot(params: {
  resolved: SudoClawConfig;
  config: SudoClawConfig;
}): ConfigFileSnapshot {
  return {
    path: "/tmp/sudoclaw.json",
    exists: true,
...
function setSnapshot(resolved: SudoClawConfig, config: SudoClawConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: SudoClawConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/sudoclaw.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
    ...
    it("handles config file path with home directory", async () => {
      ...
      snapshot.path = "/home/user/.sudoclaw/sudoclaw.json";
      mockReadConfigFileSnapshot.mockResolvedValueOnce(snapshot);

      await runConfigCommand(["config", "file"]);

      expect(mockLog).toHaveBeenCalledWith("/home/user/.sudoclaw/sudoclaw.json");
    });
  });
});
