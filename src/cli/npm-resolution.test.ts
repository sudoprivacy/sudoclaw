...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@sudoclaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@sudoclaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@sudoclaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@sudoclaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @sudoclaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@sudoclaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@sudoclaw/plugin-alpha@1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @sudoclaw/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@sudoclaw/plugin-alpha@latest",
      installPath: "/tmp/sudoclaw/extensions/alpha",
      version: "1.2.3",
