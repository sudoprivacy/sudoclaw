...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update SudoClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["sudoclaw update", "Update a source checkout (git)"],
        ["sudoclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["sudoclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["sudoclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["sudoclaw update --dry-run", "Preview actions without changing anything"],
        ["sudoclaw update --no-restart", "Update without restarting the service"],
        ["sudoclaw update --json", "Output result as JSON"],
        ["sudoclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["sudoclaw update wizard", "Interactive update wizard"],
        ["sudoclaw --update", "Shorthand for sudoclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run sudoclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.sudoclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["sudoclaw update status", "Show channel + version status."],
          ["sudoclaw update status --json", "JSON output."],
          ["sudoclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.sudoclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
