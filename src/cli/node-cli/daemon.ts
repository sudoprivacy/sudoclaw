...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("sudoclaw node install"), formatCliCommand("sudoclaw node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("sudoclaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    SUDOCLAW_LOG_PREFIX: baseEnv.SUDOCLAW_LOG_PREFIX ?? "node",
