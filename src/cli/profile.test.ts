...
describe("applyCliProfileEnv", () => {
  it("fills env defaults for dev profile", () => {
    ...
    const expectedStateDir = path.join(path.resolve("/home/peter"), ".sudoclaw-dev");
  ...
  it("uses SUDOCLAW_HOME when deriving profile state dir", () => {
    ...
    expect(env.SUDOCLAW_STATE_DIR).toBe(path.join(resolvedHome, ".sudoclaw-work"));
    expect(env.SUDOCLAW_CONFIG_PATH).toBe(
      path.join(resolvedHome, ".sudoclaw-work", "sudoclaw.json"),
    );
  });
});
...
describe("formatCliCommand", () => {
  ...
  it("trims whitespace from profile", () => {
    expect(formatCliCommand("sudoclaw doctor --fix", { SUDOCLAW_PROFILE: "  jbsudoclaw  " })).toBe(
      "sudoclaw --profile jbsudoclaw doctor --fix",
    );
  });

  it("handles command with no args after sudoclaw", () => {
