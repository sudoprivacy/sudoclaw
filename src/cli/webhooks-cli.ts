...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.sudoclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + SudoClaw hooks")
    ...
    .option("--hook-url <url>", "SudoClaw hook URL")
    .option("--hook-token <token>", "SudoClaw hook token")
