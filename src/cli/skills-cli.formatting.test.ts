...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "sudoclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "sudoclaw-bundled-skills-test-"));
    process.env.SUDOCLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
