...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop sudoclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@sudoclaw_bot", { botUsername: "sudoclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@sudoclaw_bot", { botUsername: "sudoclaw_bot" })).toBe(true);
