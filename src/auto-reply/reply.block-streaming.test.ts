...
function createReplyConfig(home: string, streamMode?: "block"): SudoClawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("sudoclaw-stream-", async (home) => {
    await fs.mkdir(path.join(home, ".sudoclaw", "agents", "main", "sessions"), { recursive: true });
    return fn(home);
  });
}
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("SUDOCLAW_TEST_FAST", "1");
