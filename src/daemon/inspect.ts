...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "sudoclaw" | "openclaw" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["sudoclaw", "openclaw", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["sudoclaw_service_marker", "openclaw_service_marker"];
  const kindKeys = ["sudoclaw_service_kind", "openclaw_service_kind"];
...
function isSudoClawGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.sudoclaw.");
}

function isSudoClawGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
...
function isSudoClawGatewayTaskName(name: string): boolean {
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("openclaw") || lower.includes("clawdbot") || lower.includes("moltbot");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (marker === "sudoclaw" && isSudoClawGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "sudoclaw" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "sudoclaw" && isSudoClawGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isSudoClawGatewayTaskName(name)) {
        continue;
      }
