...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.SUDOCLAW_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service SUDOCLAW_GATEWAY_TOKEN does not match gateway.auth.token in sudoclaw.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `sudoclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
