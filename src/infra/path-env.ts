...
function candidateBinDirs(opts: EnsureSudoClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `sudoclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "sudoclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.SUDOCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "sudoclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `sudoclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSudoClawCliOnPath(opts: EnsureSudoClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SUDOCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.SUDOCLAW_PATH_BOOTSTRAPPED = "1";
