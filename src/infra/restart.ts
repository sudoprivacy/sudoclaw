...
export function triggerSudoClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SUDOCLAW_SYSTEMD_UNIT,
        process.env.SUDOCLAW_PROFILE,
      );
  ...
  const label =
    process.env.SUDOCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SUDOCLAW_PROFILE);
