...
const triggerSudoClawRestartMock = vi.hoisted(() => vi.fn());

vi.mock("node:child_process", () => ({
  spawn: (...args: unknown[]) => spawnMock(...args),
}));
vi.mock("./restart.js", () => ({
  triggerSudoClawRestart: (...args: unknown[]) => triggerSudoClawRestartMock(...args),
}));
...
afterEach(() => {
  ...
  spawnMock.mockClear();
  triggerSudoClawRestartMock.mockClear();
  if (originalPlatformDescriptor) {
...
describe("restartGatewayProcessWithFreshPid", () => {
  ...
  it("runs launchd kickstart helper on macOS when launchd label is set", () => {
    ...
    triggerSudoClawRestartMock.mockReturnValue({ ok: true, method: "launchctl" });

    const result = restartGatewayProcessWithFreshPid();

    expect(result.mode).toBe("supervised");
    expect(triggerSudoClawRestartMock).toHaveBeenCalledOnce();
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("returns failed when launchd kickstart helper fails", () => {
    ...
    process.env.LAUNCH_JOB_LABEL = "ai.sudoclaw.gateway";
    process.env.SUDOCLAW_LAUNCHD_LABEL = "ai.sudoclaw.gateway";
    triggerSudoClawRestartMock.mockReturnValue({
      ok: false,
      method: "launchctl",
      detail: "spawn failed",
    });
  ...
  it("does not schedule kickstart on non-darwin platforms", () => {
    ...
    expect(result.mode).toBe("supervised");
    expect(triggerSudoClawRestartMock).not.toHaveBeenCalled();
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("returns supervised when SUDOCLAW_SERVICE_MARKER is set", () => {
    clearSupervisorHints();
    process.env.SUDOCLAW_SERVICE_MARKER = "gateway";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(spawnMock).not.toHaveBeenCalled();
  });
