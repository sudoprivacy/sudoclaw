...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["SUDOCLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.SUDOCLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveSudoClawPackageRoot } = await import("./sudoclaw-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveSudoClawPackageRoot).mockResolvedValue("/opt/sudoclaw");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/sudoclaw/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
