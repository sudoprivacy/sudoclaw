...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__sudoclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "sudoclaw-root");
...
describe("resolveSudoClawPackageRoot", () => {
  let resolveSudoClawPackageRoot: typeof import("./sudoclaw-root.js").resolveSudoClawPackageRoot;
  let resolveSudoClawPackageRootSync: typeof import("./sudoclaw-root.js").resolveSudoClawPackageRootSync;

  beforeAll(async () => {
    ({ resolveSudoClawPackageRoot, resolveSudoClawPackageRootSync } =
      await import("./sudoclaw-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "sudoclaw");
    const pkgRoot = path.join(project, "node_modules", "sudoclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "sudoclaw" }));

    expect(resolveSudoClawPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "sudoclaw");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "sudoclaw.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "sudoclaw" }));

    expect(resolveSudoClawPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveSudoClawPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-sudoclaw package roots", async () => {
    const pkgRoot = fx("not-sudoclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-sudoclaw" }));

    expect(resolveSudoClawPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveSudoClawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveSudoClawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
