...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: SudoClawConfig | null = null;
  ...
  get(key: string, cfg: SudoClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: SudoClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: SudoClawConfig): void {
