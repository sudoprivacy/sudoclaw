...
vi.mock("./sudoclaw-root.js", () => ({
  resolveSudoClawPackageRoot: vi.fn(async () => null),
  resolveSudoClawPackageRootSync: vi.fn(() => null),
}));
...
let sudoclawRoot: typeof import("./sudoclaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    sudoclawRoot = await import("./sudoclaw-root.js");
  });
  ...
  it("uses resolveSudoClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/sudoclaw");
    (
      sudoclawRoot.resolveSudoClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/sudoclaw"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "sudoclaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "sudoclaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-sudoclaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/sudoclaw-bundle");
    (
      sudoclawRoot.resolveSudoClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
