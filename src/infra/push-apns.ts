...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.SUDOCLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.SUDOCLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set SUDOCLAW_APNS_TEAM_ID and SUDOCLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.SUDOCLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.SUDOCLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.SUDOCLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set SUDOCLAW_APNS_PRIVATE_KEY_P8 or SUDOCLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading SUDOCLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createSudoClawPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    sudoclaw: createSudoClawPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
