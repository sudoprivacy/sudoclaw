...
import { POSIX_SUDOCLAW_TMP_DIR, resolvePreferredSudoClawTmpDir } from "./tmp-sudoclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredSudoClawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `sudoclaw-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredSudoClawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredSudoClawTmpDir", () => {
  ...
  it("falls back to os.tmpdir()/sudoclaw when /tmp/sudoclaw is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/sudoclaw when /tmp is not writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback SudoClaw temp dir/);
  });
