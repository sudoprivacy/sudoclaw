...
describe("resolveExistingInstallPath", () => {
  ...
  beforeEach(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-install-flow-"));
  });
...
describe("withExtractedArchiveRoot", () => {
  ...
  it("extracts archive and passes root directory to callback", async () => {
    const tmpRoot = path.join(path.sep, "tmp", "sudoclaw-install-flow");
    ...
    expect(withTempDirSpy).toHaveBeenCalledWith("sudoclaw-plugin-", expect.any(Function));
