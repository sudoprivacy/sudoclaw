...
const DEFAULT_PACKAGE_NAME = "sudoclaw";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "sudoclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("sudoclaw doctor", doctorArgv, gitRoot, { SUDOCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
