...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(
    ...
    await fs.mkdir(path.join(base, ".sudoclaw", "agents", "main", "sessions"), { recursive: true });
  ...
  async function writeAuthProfiles(home: string, profiles: Record<string, unknown>) {
    const agentDir = path.join(home, ".sudoclaw", "agents", "main", "agent");
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    const stateDir = path.join(home, ".sudoclaw");
    await fs.mkdir(stateDir, { recursive: true });
    await fs.writeFile(
      path.join(stateDir, "sudoclaw.json"),
