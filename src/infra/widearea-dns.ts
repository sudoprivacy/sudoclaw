...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SUDOCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*sudoclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "sudoclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "sudoclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "sudoclaw-gw");
  ...
  records.push(`_sudoclaw-gw._tcp IN PTR ${instanceLabel}._sudoclaw-gw._tcp`);
  records.push(`${instanceLabel}._sudoclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._sudoclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; sudoclaw-content-hash: ${contentHash}\n${contentBody}`;
}
