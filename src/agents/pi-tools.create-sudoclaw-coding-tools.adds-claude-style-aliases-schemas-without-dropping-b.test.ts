...
const defaultTools = createSudoClawCodingTools({ senderIsOwner: true });
...
describe("createSudoClawCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: SudoClawConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createSudoClawCodingTools({
    ...
    const anthropicTools = createSudoClawCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: SudoClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createSudoClawCodingTools({
    ...
    const denied = createSudoClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createSudoClawCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
