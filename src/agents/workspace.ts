...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.SUDOCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".sudoclaw", `workspace-${profile}`);
  }
  return path.join(home, ".sudoclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".sudoclaw";
