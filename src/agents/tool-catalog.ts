...
type CoreToolDefinition = {
  ...
  includeInSudoClawGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const sudoclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInSudoClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:sudoclaw": sudoclawTools,
