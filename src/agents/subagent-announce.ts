...
const FAST_TEST_MODE = process.env.SUDOCLAW_TEST_FAST === "1";
    ...
    lines.push(
      ...
      ...(acpEnabled
        ? [
            ...
            '`agents_list` and `subagents` apply to SudoClaw sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`sudoclaw ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for SudoClaw subagents (`runtime: "subagent"`).',
