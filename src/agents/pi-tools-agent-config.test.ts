...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: SudoClawConfig) {
    return createSudoClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<SudoClawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<SudoClawConfig["agents"]>["list"]>[number]["tools"];
  }): SudoClawConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createSudoClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createSudoClawCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createSudoClawCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createSudoClawCodingTools({
      config: cfg,
    ...
    const bobTools = createSudoClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createSudoClawCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createSudoClawCodingTools({
      config: cfg,
