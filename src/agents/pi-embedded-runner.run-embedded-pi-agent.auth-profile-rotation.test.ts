...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureSudoClawModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): SudoClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): SudoClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): SudoClawConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-workspace-"));
    const now = Date.now();
