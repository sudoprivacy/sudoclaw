...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("sudoclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("sudoclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("sudoclaw-sbx-");
