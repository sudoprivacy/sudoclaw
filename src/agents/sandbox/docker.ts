...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "sudoclaw.sandbox=1");
  args.push("--label", `sudoclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `sudoclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `sudoclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "sudoclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`sudoclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`sudoclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("sudoclaw sandbox recreate --all");
}
