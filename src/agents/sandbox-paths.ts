...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const sudoClawTmpDir = path.resolve(resolvePreferredSudoClawTmpDir());
  if (!isPathInside(sudoClawTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: sudoClawTmpDir });
  return resolved;
}
