...
test("exec sets SUDOCLAW_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-sudoclaw-shell", {
    command: "node -e \"process.stdout.write(process.env.SUDOCLAW_SHELL || '')\"",
    pty: true,
  });
