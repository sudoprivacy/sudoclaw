...
import { __testing, createSudoClawCodingTools } from "./pi-tools.js";
import { createSudoClawReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
import { createSudoClawTools } from "./sudoclaw-tools.js";
import { createHostSandboxFsBridge } from "./test-helpers/host-sandbox-fs-bridge.js";
import { createBrowserTool } from "./tools/browser-tool.js";

const defaultTools = createSudoClawCodingTools();
...
describe("createSudoClawCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createSudoClawCodingTools({ senderIsOwner: true }).find(
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-depth-policy-"));
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    ...
    const googleTools = createSudoClawCodingTools({
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "sudoclaw-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createSudoClawReadTool(
      baseRead as unknown as Parameters<typeof createSudoClawReadTool>[0],
    );
