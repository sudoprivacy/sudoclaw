...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["SUDOCLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-subagent-"));
  ...
  it("uses isolated temp state when SUDOCLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "sudoclaw-test-state"));
  });
});
