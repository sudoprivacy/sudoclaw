...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: SudoClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/sudoclaw" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: SudoClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: SudoClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: SudoClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: SudoClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: SudoClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: SudoClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: SudoClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/sudoclaw");
  });
  ...
  it("uses SUDOCLAW_HOME for default agent workspace", () => {
    ...
    const workspace = resolveAgentWorkspaceDir({} as SudoClawConfig, "main");
  ...
  it("uses SUDOCLAW_HOME for default agentDir", () => {
    ...
    const agentDir = resolveAgentDir({} as SudoClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".sudoclaw", "agents", "main", "agent"));
  });
});
