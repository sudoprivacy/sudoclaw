...
const GOOGLE_LIVE_SINGLE_KEY = "SUDOCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "SUDOCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "SUDOCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "SUDOCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `SUDOCLAW_LIVE_${base}_KEY`;
