...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.SUDOCLAW_TEST_FAST;
...
describe("sudoclaw-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    ...
    process.env.SUDOCLAW_TEST_FAST = fastModeEnv.previous;
  });
