...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("sudoclaw-ws-", async (workspaceDir) => {
      await withTempDir("sudoclaw-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createSudoClawCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("sudoclaw-ws-", async (workspaceDir) => {
      await withTempDir("sudoclaw-override-", async (overrideDir) => {
        const tools = createSudoClawCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("sudoclaw-ws-", async (workspaceDir) => {
      const cfg: SudoClawConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createSudoClawCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-sudoclaw.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("sudoclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("sudoclaw-workspace-", async (workspaceDir) => {
        ...
        const tools = createSudoClawCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
