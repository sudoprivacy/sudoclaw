...
describe("createSudoClawCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-read-"));
    try {
      ...
      const contents = "Hello from sudoclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createSudoClawCodingTools({ sandbox });
