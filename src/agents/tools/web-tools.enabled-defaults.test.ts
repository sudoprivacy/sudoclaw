...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "sudoclaw" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [
              { title: "SudoClaw", url: "https://sudoclaw.ai/docs", content: "docs" },
            ],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest sudoclaw release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://sudoclaw.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://sudoclaw.ai/docs"]);
