...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): SudoClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as SudoClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): SudoClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as SudoClawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: SudoClawConfig, accountId = "media") {
    await handleTelegramAction(
