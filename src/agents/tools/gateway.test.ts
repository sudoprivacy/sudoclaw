...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    sudoclaw: process.env.SUDOCLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.sudoclaw === undefined) {
      delete process.env.SUDOCLAW_GATEWAY_TOKEN;
    } else {
      process.env.SUDOCLAW_GATEWAY_TOKEN = envSnapshot.sudoclaw;
    }
  ...
  it("uses SUDOCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.SUDOCLAW_GATEWAY_TOKEN = "local-env-token";
