...
type ModelsConfig = NonNullable<SudoClawConfig["models"]>;
...
export async function ensureSudoClawModelsJson(
  config?: SudoClawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveSudoClawAgentDir();
