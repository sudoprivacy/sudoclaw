...
import { resolvePreferredSudoClawTmpDir } from "../infra/tmp-sudoclaw-dir.js";
...
async function withOutsideHardlinkInSudoClawTmp<T>(
  params: {
    sudoClawTmpDir: string;
  ...
  const hardlinkPath = path.join(params.sudoClawTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.sudoClawTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.sudoClawTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.sudoClawTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const sudoClawTmpDir = resolvePreferredSudoClawTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred SudoClaw tmp root",
      media: path.join(sudoClawTmpDir, "image.png"),
      expected: path.join(sudoClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred SudoClaw tmp root",
      media: pathToFileURL(path.join(sudoClawTmpDir, "photo.png")).href,
      expected: path.join(sudoClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred SudoClaw tmp root",
      media: path.join(sudoClawTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(sudoClawTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(sudoClawTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside sudoclaw tmp root",
      media: path.join(os.tmpdir(), "outside-sudoclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked SudoClaw tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(sudoClawTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(sudoClawTmpDir, { recursive: true });
      const symlinkPath = path.join(sudoClawTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked SudoClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInSudoClawTmp(
      {
        sudoClawTmpDir,
  ...
  it("rejects symlinked SudoClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInSudoClawTmp(
