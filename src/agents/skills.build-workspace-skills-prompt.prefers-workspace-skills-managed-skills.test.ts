...
const fixtureSuite = createFixtureSuite("sudoclaw-skills-prompt-suite-");
...
describe("buildWorkspaceSkillsPrompt", () => {
  ...
  it("gates by bins, config, and always", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"sudoclaw":{"requires":{"bins":["fakebin"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "anybin-skill"),
      name: "anybin-skill",
      description: "Needs any bin",
      metadata: '{"sudoclaw":{"requires":{"anyBins":["missingbin","fakebin"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "config-skill"),
      name: "config-skill",
      description: "Needs config",
      metadata: '{"sudoclaw":{"requires":{"config":["browser.enabled"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "always-skill"),
      name: "always-skill",
      description: "Always on",
      metadata: '{"sudoclaw":{"always":true,"requires":{"env":["MISSING"]}}}',
    });
    await writeSkill({
  ...
  it("uses skillKey for config lookups", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"sudoclaw":{"skillKey":"alias"}}',
    });
