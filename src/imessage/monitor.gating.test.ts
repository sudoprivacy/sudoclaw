...
function baseCfg(): SudoClawConfig {
  return {
    channels: {
      imessage: {
        dmPolicy: "open",
...
function buildDispatchContextPayload(params: { cfg: SudoClawConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@sudoclaw hello",
      bodyText: "@sudoclaw hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
  ...
  it("honors group allowlist and ignores pairing-store senders in groups", () => {
    ...
    const denied = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@sudoclaw hi",
      bodyText: "@sudoclaw hi",
      allowFrom: ["*"],
    ...
    const allowed = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@sudoclaw ok",
      bodyText: "@sudoclaw ok",
      allowFrom: ["*"],
      groupAllowFrom: ["chat_id:101"],
      groupPolicy: "allowlist",
      dmPolicy: "pairing",
