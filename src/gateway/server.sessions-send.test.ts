...
const { createSudoClawTools } = await import("../agents/sudoclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createSudoClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createSudoClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["SUDOCLAW_GATEWAY_PORT", "SUDOCLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.SUDOCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.SUDOCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("SUDOCLAW_CONFIG_PATH missing in gateway test environment");
      }
