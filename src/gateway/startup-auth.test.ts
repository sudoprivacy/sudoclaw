...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: SudoClawConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: SudoClawConfig) {
  ...
  async function expectNoTokenGeneration(cfg: SudoClawConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          SUDOCLAW_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
