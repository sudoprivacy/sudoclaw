...
export const setTestConfigRoot = (root: string) => {
  ...
  process.env.SUDOCLAW_CONFIG_PATH = path.join(root, "sudoclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "sudoclaw.json");
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadSudoClawPlugins: () => pluginRegistryState.registry,
  };
});
