...
import { resolveSudoClawAgentDir } from "../agents/agent-paths.js";
...
import { ensureSudoClawModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, SudoClawConfig, ModelProviderConfig } from "../config/types.js";
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<SudoClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveSudoClawAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-live-state-"));
  process.env.SUDOCLAW_STATE_DIR = tempStateDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.sudoclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveSudoClawAgentDir();
  const sanitizedCfg: SudoClawConfig = {
    ...params.cfg,
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "SudoClaw live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.sudoclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
