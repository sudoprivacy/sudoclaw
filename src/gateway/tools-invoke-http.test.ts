...
let lastCreateSudoClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/sudoclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateSudoClawToolsContext?.agentTo,
          agentThreadId: lastCreateSudoClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createSudoClawTools: (ctx: Record<string, unknown>) => {
      lastCreateSudoClawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateSudoClawToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-sudoclaw-message-to": "channel:24514",
        "x-sudoclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
