...
beforeAll(async () => {
  ...
  sharedSessionStoreDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-sessions-"));
...
describe("gateway server sessions", () => {
  ...
  test("control-ui client can delete sessions even in webchat mode", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-sessions-control-ui-delete-"));
