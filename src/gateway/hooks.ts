...
export function resolveHooksConfig(cfg: SudoClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SudoClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-sudoclaw-token"] === "string"
      ? req.headers["x-sudoclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
