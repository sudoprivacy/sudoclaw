...
const CLI_LIVE = isTruthyEnvValue(process.env.SUDOCLAW_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.SUDOCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.SUDOCLAW_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("SUDOCLAW_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    const rawModel = process.env.SUDOCLAW_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `SUDOCLAW_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliCommand = process.env.SUDOCLAW_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(
        `SUDOCLAW_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
      );
    }
    const baseCliArgs =
      parseJsonStringArray(
        "SUDOCLAW_LIVE_CLI_BACKEND_ARGS",
        process.env.SUDOCLAW_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`SUDOCLAW_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "SUDOCLAW_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.SUDOCLAW_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.SUDOCLAW_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.SUDOCLAW_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "SUDOCLAW_LIVE_CLI_BACKEND_IMAGE_MODE requires SUDOCLAW_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-live-cli-"));
    const disableMcpConfig = process.env.SUDOCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
    ...
    const tempConfigPath = path.join(tempDir, "sudoclaw.json");
    await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
    process.env.SUDOCLAW_CONFIG_PATH = tempConfigPath;
    ...
    try {
      ...
      if (previous.skipChannels === undefined) {
        delete process.env.SUDOCLAW_SKIP_CHANNELS;
      } else {
        process.env.SUDOCLAW_SKIP_CHANNELS = previous.skipChannels;
      }
      if (previous.skipGmail === undefined) {
        delete process.env.SUDOCLAW_SKIP_GMAIL_WATCHER;
      } else {
      ...
      if (previous.skipCanvas === undefined) {
        delete process.env.SUDOCLAW_SKIP_CANVAS_HOST;
      } else {
