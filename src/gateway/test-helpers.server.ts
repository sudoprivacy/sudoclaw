...
function applyGatewaySkipEnv() {
  ...
  process.env.SUDOCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "sudoclaw-test-no-bundled-extensions")
    : "sudoclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "sudoclaw-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".sudoclaw-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__sudoclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__sudoclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["SUDOCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.SUDOCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SUDOCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.SUDOCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
