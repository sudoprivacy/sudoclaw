...
vi.mock("./chrome.js", () => ({
  resolveSudoClawUserDataDir: vi.fn(() => "/tmp/sudoclaw-test/sudoclaw/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveSudoClawUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "sudoclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveSudoClawUserDataDir).mockReturnValue(userDataDir);
