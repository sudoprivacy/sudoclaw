...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const sudoclaw = resolveProfile(resolved, "sudoclaw");
    expect(sudoclaw?.driver).toBe("sudoclaw");
    expect(sudoclaw?.cdpPort).toBe(18800);
    expect(sudoclaw?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from SUDOCLAW_GATEWAY_PORT when unset", () => {
    withEnv({ SUDOCLAW_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const sudoclaw = resolveProfile(resolved, "sudoclaw");
      expect(sudoclaw?.cdpPort).toBe(19012);
      expect(sudoclaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ SUDOCLAW_GATEWAY_PORT: undefined }, () => {
      ...
      const sudoclaw = resolveProfile(resolved, "sudoclaw");
      expect(sudoclaw?.cdpPort).toBe(19022);
      expect(sudoclaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "sudoclaw");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        sudoclaw: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("sudoclaw");
  });
