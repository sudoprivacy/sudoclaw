import type { BrowserConfig, BrowserProfileConfig, SudoClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_SUDOCLAW_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "sudoclaw" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_SUDOCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "sudoclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SUDOCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SUDOCLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an SudoClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for sudoclaw/sudoclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: SudoClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SUDOCLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "sudoclaw";
