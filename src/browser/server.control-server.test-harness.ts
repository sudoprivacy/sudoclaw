...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.SUDOCLAW_GATEWAY_PORT = prevGatewayPort;
}
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          sudoclaw: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchSudoClawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveSudoClawUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopSudoClawChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.SUDOCLAW_GATEWAY_PORT;
  process.env.SUDOCLAW_GATEWAY_PORT = String(state.testPort - 2);
