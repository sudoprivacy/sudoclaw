...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    // Start with only sudoclaw profile
    ...
    // 2. Simulate adding a new profile to config (like user editing sudoclaw.json)
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    cfgProfiles.sudoclaw = { cdpPort: 19999, color: "#FF4500" };
    cachedConfig = null;
    ...
    expect(state.resolved.profiles.sudoclaw?.cdpPort).toBe(19999);
  });
