...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredSudoClawTmpDir: vi.fn(() => "/tmp/sudoclaw"),
}));
vi.mock("../infra/tmp-sudoclaw-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredSudoClawTmpDir.mockReturnValue("/tmp/sudoclaw");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-browser-download-test-"));
    try {
  ...
  it("waits for the next download and atomically finalizes explicit output paths", async () => {
    await withTempDir(async (tempDir) => {
      ...
      expect(path.basename(String(savedPath))).toContain(".sudoclaw-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredSudoClawTmpDir.mockReturnValue("/tmp/sudoclaw-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "sudoclaw-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "sudoclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredSudoClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "sudoclaw-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "sudoclaw-preferred", "downloads")}${path.sep}`),
    );
  });
