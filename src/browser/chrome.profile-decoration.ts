...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".sudoclaw-profile-decorated");
}
 ...
 */
export function decorateSudoClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SUDOCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SUDOCLAW_BROWSER_COLOR).toUpperCase();
