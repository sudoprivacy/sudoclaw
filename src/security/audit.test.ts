...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: SudoClawConfig, accountId: string | null | undefined) => unknown;
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync(
      { SUDOCLAW_STATE_DIR: channelSecurityStateDir },
      async () => await fn(channelSecurityStateDir),
    );
  };

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-security-audit-"));
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("sudoclaw-sbx-browser-old\nsudoclaw-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "sudoclaw-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "sudoclaw-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("sudoclaw-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("sudoclaw-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "sudoclaw-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "sudoclaw-sbx-browser-exposed") {
        return {
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: SudoClawConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: SudoClawConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): SudoClawConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: SudoClawConfig = { session: { dmScope: "main" } };
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.sudoclaw/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: SudoClawConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.SUDOCLAW_GATEWAY_TOKEN;
    process.env.SUDOCLAW_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: SudoClawConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<SudoClawConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: SudoClawConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.sudoclaw",
      configPath: "/Users/test/Dropbox/.sudoclaw/sudoclaw.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: SudoClawConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
    ...
    const cfg: SudoClawConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@sudoclaw/voice-call@1.2.3",
      ...
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@sudoclaw/test-hooks@1.2.3",
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@sudoclaw/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@sudoclaw/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("does not scan plugin code safety findings when deep audit is disabled", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        sudoclaw: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        sudoclaw: { extensions: ["../outside.js"] },
      }),
    );
