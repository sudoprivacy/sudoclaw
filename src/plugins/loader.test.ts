...
import { __testing, loadSudoClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `sudoclaw-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.SUDOCLAW_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "sudoclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.SUDOCLAW_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadSudoClawPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadSudoClawPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.SUDOCLAW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.SUDOCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadSudoClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadSudoClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadSudoClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.SUDOCLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadSudoClawPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadSudoClawPlugins(options);
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ SUDOCLAW_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
