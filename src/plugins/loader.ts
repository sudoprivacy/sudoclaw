...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SudoClawPluginDefinition;
  register?: SudoClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SudoClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as SudoClawPluginDefinition;
...
export function loadSudoClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverSudoClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "sudoclaw/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "sudoclaw/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: SudoClawPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as SudoClawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
