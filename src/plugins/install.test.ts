...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `sudoclaw-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        sudoclaw: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function createZipperArchiveBuffer(): Promise<Buffer> {
  ...
  zip.file(
    ...
    JSON.stringify({
      name: "@sudoclaw/zipper",
      version: "0.0.1",
      sudoclaw: { extensions: ["./dist/index.js"] },
    }),
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.sudoclaw/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without sudoclaw.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@sudoclaw/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("sudoclaw.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.sudoclaw).toBeUndefined();
  ...
  it("uses sudoclaw.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "cognee-sudoclaw"',
        ),
      ),
    ).toBe(true);
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@sudoclaw/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@sudoclaw/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@sudoclaw/voice-call@0.0.1",
    });
