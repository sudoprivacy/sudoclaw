...
import { discoverSudoClawPlugins } from "./discovery.js";

const tempDirs: string[] = [];

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `sudoclaw-plugins-${randomUUID()}`);
...
describe("discoverSudoClawPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".sudoclaw", "extensions");
    fs.mkdirSync(workspaceExt, { recursive: true });
    fs.writeFileSync(path.join(workspaceExt, "beta.ts"), "export default function () {}", "utf-8");

    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverSudoClawPlugins({ workspaceDir });
    });
  ...
  it("ignores backup and disabled plugin directories in scanned roots", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverSudoClawPlugins({});
    });
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "pack",
        sudoclaw: { extensions: ["./src/one.ts", "./src/two.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        sudoclaw: { extensions: ["./src/index.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverSudoClawPlugins({ extraPaths: [packDir] });
    });
  ...
  it("rejects package extension entries that escape via symlink", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@sudoclaw/pack",
        sudoclaw: { extensions: ["./linked/escape.ts"] },
      }),
      "utf-8",
    );
  ...
  it.runIf(process.platform !== "win32" && typeof process.getuid === "function")(
    ...
    async () => {
      ...
      const result = await withStateDir(stateDir, async () => {
        return discoverSudoClawPlugins({ ownershipUid: actualUid + 1 });
      });
      expect(result.candidates).toHaveLength(0);
