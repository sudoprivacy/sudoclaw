...
describe("canvas host state dir defaults", () => {
  it("uses SUDOCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("sudoclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("SudoClaw Canvas");
      } finally {
        await handler.close();
      }
    });
