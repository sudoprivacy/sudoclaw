...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const previousStateDir = process.env.SUDOCLAW_STATE_DIR;

    const tempHome = await createTempHomeEnv("sudoclaw-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.SUDOCLAW_STATE_DIR).toBe(path.join(tempHome.home, ".sudoclaw"));
    ...
    expect(process.env.SUDOCLAW_STATE_DIR).toBe(previousStateDir);
