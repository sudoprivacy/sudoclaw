...
`;
    ...
    expect(parsed.sudoclaw.emoji).toBe("💾");
    expect(parsed.sudoclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.sudoclaw.emoji).toBe("📝");
    expect(parsed.sudoclaw.events).toEqual(["command"]);
    expect(parsed.sudoclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.sudoclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"sudoclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"sudoclaw": {"events": ["test"]}}');
  });
...
describe("resolveSudoClawMetadata", () => {
  it("extracts sudoclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveSudoClawMetadata(frontmatter);
  ...
  it("returns undefined when sudoclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        sudoclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with SudoClaw" },
            { id: "npm", kind: "npm", package: "@sudoclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@sudoclaw/hook");
  });
    ...
    "sudoclaw":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with SudoClaw" }],
      },
  }
---
...
`;
    ...
    const sudoclaw = resolveSudoClawMetadata(frontmatter);
    expect(sudoclaw).toBeDefined();
    expect(sudoclaw?.emoji).toBe("💾");
    expect(sudoclaw?.events).toEqual(["command:new", "command:reset"]);
    expect(sudoclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(sudoclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const sudoclaw = resolveSudoClawMetadata(frontmatter);
    expect(sudoclaw?.emoji).toBe("disk");
    expect(sudoclaw?.events).toEqual(["command:new"]);
  });
});
