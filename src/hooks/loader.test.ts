...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["SUDOCLAW_BUNDLED_HOOKS_DIR"]);
    process.env.SUDOCLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
