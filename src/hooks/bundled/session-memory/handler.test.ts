...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => SudoClawConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("sudoclaw-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): SudoClawConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
