...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ SUDOCLAW_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.SUDOCLAW_BASE_ENV ?? "") + "|" + (process.env.SUDOCLAW_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { SUDOCLAW_TEST_ENV: "ok" },
        },
      );
