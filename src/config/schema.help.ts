...
export const FIELD_HELP: Record<string, string> = {
  meta: "Metadata fields automatically maintained by SudoClaw to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
  "meta.lastTouchedVersion": "Auto-set when SudoClaw writes the config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for SudoClaw. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
  ...
  "wizard.lastRunVersion":
    "SudoClaw version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version onboarding changes.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping SudoClaw runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  ...
  "agents.defaults.sandbox.browser.network":
    "Docker network for sandbox browser containers (default: sudoclaw-sandbox-browser). Avoid bridge if you need stricter isolation.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /sudoclaw).",
  ...
  "gateway.nodes.allowCommands":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `sudoclaw security audit`.",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $SUDOCLAW_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "models.providers.*.injectNumCtxForOpenAICompat":
    "Controls whether SudoClaw injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
  ...
  "agents.defaults.memorySearch.store.path":
    "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.sudoclaw/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
  ...
  "memory.backend":
    'Selects the global memory engine: "builtin" uses SudoClaw memory internals, while "qmd" uses the QMD sidecar pipeline. Keep "builtin" unless you intentionally operate QMD.',
  "memory.citations":
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `sudoclaw plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.sudoclaw/extensions/<id>).",
  ...
  "agents.defaults.embeddedPi":
    "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in SudoClaw sessions.",
  ...
  "session.parentForkMaxTokens":
    "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, SudoClaw starts a fresh thread session instead of forking; set 0 to disable this protection.",
  "session.mainKey":
  ...
  hooks:
    "Inbound webhook automation surface for mapping external events into wake or agent actions in SudoClaw. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
  "hooks.enabled":
  ...
  "channels.irc":
    "IRC channel provider configuration and compatibility settings for classic IRC transport workflows. Use this section when bridging legacy chat infrastructure into SudoClaw.",
  "channels.defaults":
  ...
  "channels.discord.eventQueue.listenerTimeout":
    "Canonical Discord listener timeout control in ms for gateway event handlers. Default is 120000 in SudoClaw; set per account via channels.discord.accounts.<id>.eventQueue.listenerTimeout.",
