...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("sudoclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "sudoclaw-sandbox-browser-global" },
      agentBrowser: { network: "sudoclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("sudoclaw-sandbox-browser-agent");
  });
