...
import type { SudoClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareSudoClawVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  sudoclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  sudoclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: SudoClawConfig,
  pathSegments: string[],
): { changed: boolean; next: SudoClawConfig } {
...
function coerceConfig(value: unknown): SudoClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SudoClawConfig;
}
...
function stampConfigVersion(cfg: SudoClawConfig): SudoClawConfig {
...
function warnIfConfigFromFuture(cfg: SudoClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareSudoClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer SudoClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as SudoClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): SudoClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as SudoClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: SudoClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as SudoClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.SUDOCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.SUDOCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.SUDOCLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.SUDOCLAW_WATCH_SESSION === "string" &&
        deps.env.SUDOCLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.SUDOCLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.SUDOCLAW_WATCH_COMMAND === "string" &&
        deps.env.SUDOCLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.SUDOCLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `SUDOCLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: SudoClawConfig | null = null;
let runtimeConfigSourceSnapshot: SudoClawConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SUDOCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SUDOCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): SudoClawConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): SudoClawConfig {
