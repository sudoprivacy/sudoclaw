...
describe("Nix integration (U3, U5, U9)", () => {
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    it("STATE_DIR defaults to ~/.sudoclaw when env not set", () => {
      expect(resolveStateDir(envWith({ SUDOCLAW_STATE_DIR: undefined }))).toMatch(/\.sudoclaw$/);
    });

    it("STATE_DIR respects SUDOCLAW_STATE_DIR override", () => {
      expect(resolveStateDir(envWith({ SUDOCLAW_STATE_DIR: "/custom/state/dir" }))).toBe(
    ...
    it("CONFIG_PATH defaults to SUDOCLAW_HOME/.sudoclaw/sudoclaw.json", () => {
      ...
      ).toBe(path.join(path.resolve(customHome), ".sudoclaw", "sudoclaw.json"));
    });

    it("CONFIG_PATH defaults to ~/.sudoclaw/sudoclaw.json when env not set", () => {
      expect(
        resolveConfigPathCandidate(
          envWith({ SUDOCLAW_CONFIG_PATH: undefined, SUDOCLAW_STATE_DIR: undefined }),
        ),
      ).toMatch(/\.sudoclaw[\\/]sudoclaw\.json$/);
    });
    ...
    it("CONFIG_PATH uses STATE_DIR when only state dir is overridden", () => {
      expect(resolveConfigPathCandidate(envWith({ SUDOCLAW_STATE_DIR: "/custom/state" }))).toBe(
        path.join(path.resolve("/custom/state"), "sudoclaw.json"),
      );
    });
  });
  ...
  describe("U5b: tilde expansion for config paths", () => {
    it("expands ~ in common path-ish config fields", async () => {
      await withTempHome(async (home) => {
        const configDir = path.join(home, ".sudoclaw");
          ...
          JSON.stringify(
            {
              ...
              channels: {
                whatsapp: {
                  accounts: {
                    personal: {
                      authDir: "~/.sudoclaw/credentials/wa-personal",
                    },
                  },
                },
              },
        ...
        expect(cfg.agents?.list?.[0]?.agentDir).toBe(
          path.join(home, ".sudoclaw", "agents", "main"),
        );
        expect(cfg.agents?.list?.[0]?.sandbox?.workspaceRoot).toBe(path.join(home, "sandbox-root"));
        expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
          path.join(home, ".sudoclaw", "credentials", "wa-personal"),
        );
      });
    });
  });
  ...
  describe("U6: gateway port resolution", () => {
    it("uses default when env and config are unset", () => {
      expect(resolveGatewayPort({}, envWith({ SUDOCLAW_GATEWAY_PORT: undefined }))).toBe(
        DEFAULT_GATEWAY_PORT,
      );
    });

    it("prefers SUDOCLAW_GATEWAY_PORT over config", () => {
      expect(
        resolveGatewayPort(
          { gateway: { port: 19002 } },
          envWith({ SUDOCLAW_GATEWAY_PORT: "19001" }),
        ),
      ).toBe(19001);
    });
    ...
    it("falls back to config when env is invalid", () => {
      expect(
        resolveGatewayPort(
          { gateway: { port: 19003 } },
          envWith({ SUDOCLAW_GATEWAY_PORT: "nope" }),
        ),
      ).toBe(19003);
    });
  });
