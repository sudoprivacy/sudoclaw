...
function isWhatsAppConfigured(cfg: SudoClawConfig): boolean {
...
function isGenericChannelConfigured(cfg: SudoClawConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: SudoClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: SudoClawConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: SudoClawConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: SudoClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: SudoClawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: SudoClawConfig, pluginId: string): SudoClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
