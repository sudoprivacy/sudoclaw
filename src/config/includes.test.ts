...
function etcSudoClawPath(...parts: string[]) {
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcSudoClawPath("agents.json");
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-includes-symlink-"));
      try {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "sudoclaw.json"),
        );
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-includes-hardlink-"));
      try {
    ...
    it("rejects oversized include files", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sudoclaw-includes-big-"));
      try {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "sudoclaw.json")),
