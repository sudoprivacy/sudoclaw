...
const TEMP_MEDIA_PREFIX = "sudoclaw-media-";
...
function createGroqAudioConfig(): SudoClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): SudoClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): SudoClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: SudoClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredSudoClawTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        SUDOCLAW_AGENT_DIR: isolatedAgentDir,
