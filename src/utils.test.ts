...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("sudoclaw-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("sudoclaw-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("sudoclaw-lid-a-", (first) => {
      withTempDirSync("sudoclaw-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.sudoclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "sudoclaw-config-dir-"));
    try {
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/sudoclaw")).toBe(path.resolve(os.homedir(), "sudoclaw"));
  });
  ...
  it("prefers SUDOCLAW_HOME for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/sudoclaw")).toBe(path.resolve("/srv/sudoclaw-home", "sudoclaw"));
