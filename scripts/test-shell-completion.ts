 ...
 * `sudoclaw update`. Use it to verify the completion installation flow
...
\`sudoclaw update\`, \`sudoclaw doctor\`, and \`sudoclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# SudoClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`sudoclaw completion --install\` later to enable.`));
    return;
  }
