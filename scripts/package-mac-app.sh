...
# Build and bundle SudoClaw into a minimal .app we can open.
# Outputs to dist/SudoClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/SudoClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="SudoClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.sudoclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/sudoclaw/sudoclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/SudoClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SudoClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SudoClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/SudoClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/SudoClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/SudoClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/SudoClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/SudoClaw/Resources/SudoClaw.icns" "$APP_ROOT/Contents/Resources/SudoClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/SudoClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SudoClawKit resources"
SUDOCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SudoClawKit_SudoClawKit.bundle"
if [ -d "$SUDOCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SudoClawKit_SudoClawKit.bundle"
  cp -R "$SUDOCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SudoClawKit_SudoClawKit.bundle"
else
  echo "WARN: SudoClawKit resource bundle not found at $SUDOCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running SudoClaw"
killall -q SudoClaw 2>/dev/null || true
