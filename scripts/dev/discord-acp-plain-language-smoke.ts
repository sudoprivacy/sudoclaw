...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver sudoclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) SudoClaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|sudoclaw> Driver transport mode (default: token)\n" +
    ...
    "  --sudoclaw-bin <path>        SudoClaw CLI binary for driver=sudoclaw (default: sudoclaw)\n" +
...
function parseArgs(): Args {
  ...
  const threadBindingsPath =
    ...
    defaultBindingsPath;
  const sudoclawBin =
    resolveArg("--sudoclaw-bin") || process.env.SUDOCLAW_DISCORD_SMOKE_SUDOCLAW_BIN || "sudoclaw";
...
async function sudoclawCliJson<T>(params: { sudoclawBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.sudoclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`sudoclaw ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithSudoclaw(params: {
  sudoclawBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await sudoclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    sudoclawBin: params.sudoclawBin,
    args: [
      "message",
      "read",
      "--channel",
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `sudoclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await sudoclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
  ...
  try {
      ...
      try {
        parentRecent =
          args.driverMode === "sudoclaw"
            ? await readMessagesWithSudoclaw({
                sudoclawBin: args.sudoclawBin,
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from SudoClaw.`,
        diagnostics: {
          bindingCandidates: [
            {
