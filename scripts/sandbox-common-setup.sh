...
BASE_IMAGE="${BASE_IMAGE:-sudoclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-sudoclaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=sudoclaw.sandbox=1)
NOTE
