...
IMAGE_NAME="sudoclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="sudoclaw-net-e2e-$$"
GW_NAME="sudoclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "SUDOCLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "SUDOCLAW_SKIP_CHANNELS=1" \
  -e "SUDOCLAW_SKIP_GMAIL_WATCHER=1" \
  -e "SUDOCLAW_SKIP_CRON=1" \
  -e "SUDOCLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
