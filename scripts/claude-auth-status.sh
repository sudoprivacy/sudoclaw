...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    if [ "$USE_JSON" -eq 1 ]; then
        ...
        openclaw_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$SUDOCLAW_AUTH" 2>/dev/null || echo "0")
    fi
...
echo ""
echo "OpenClaw Auth (~/.sudoclaw/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
