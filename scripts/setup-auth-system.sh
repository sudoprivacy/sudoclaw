#!/bin/bash
# Setup SudoClaw Auth Management System
...
echo "=== SudoClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'sudoclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  SudoClaw message: Send warning via SudoClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/sudoclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/sudoclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/sudoclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now sudoclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status sudoclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
