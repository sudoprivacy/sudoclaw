...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/sudoclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "SudoClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/sudoclaw/scripts/mobile-reauth.sh'" --id sudoclaw-auth
        ;;
    *)
