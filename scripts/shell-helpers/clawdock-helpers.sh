#!/usr/bin/env bash
# ClawDock - Docker helpers for SudoClaw
# Inspired by Simon Willison's "Running SudoClaw in Docker"
# https://til.simonwillison.net/llms/sudoclaw-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/sudoclaw/sudoclaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for SudoClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/sudoclaw"
  "${HOME}/workspace/sudoclaw"
  "${HOME}/projects/sudoclaw"
  "${HOME}/dev/sudoclaw"
  "${HOME}/code/sudoclaw"
  "${HOME}/src/sudoclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found SudoClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/sudoclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ SudoClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/sudoprivacy/sudoclaw.git ~/sudoclaw"
    echo "  cd ~/sudoclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^SUDOCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d sudoclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart sudoclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f sudoclaw-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec sudoclaw-gateway \
    bash -c 'echo "alias sudoclaw=\"./sudoclaw.mjs\"" > /tmp/.bashrc_sudoclaw && bash --rcfile /tmp/.bashrc_sudoclaw'
}
...
# Maintenance
clawdock-rebuild() {
  _clawdock_compose build sudoclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "SUDOCLAW_GATEWAY_TOKEN=$token" sudoclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" sudoclaw-gateway \
    bash -c './sudoclaw.mjs config set gateway.remote.token "$TOKEN" && ./sudoclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec sudoclaw-gateway \
    bash -c "./sudoclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart sudoclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec sudoclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'sudoclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for SudoClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (sudoclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to sudoclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.sudoclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'sudoclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'sudoclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.sudoclaw.ai${_CLR_RESET}"
  echo ""
}
