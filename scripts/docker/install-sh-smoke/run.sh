...
INSTALL_URL="${SUDOCLAW_INSTALL_URL:-https://sudoclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SUDOCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SUDOCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="sudoclaw"
PACKAGE_NAME="${SUDOCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SUDOCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SUDOCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SUDOCLAW_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
