...
#
# Suggested cron: */30 * * * * /home/admin/sudoclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send SudoClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., sudoclaw-alerts)
...
STATE_FILE="$HOME/.sudoclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via SudoClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use sudoclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via SudoClaw to $NOTIFY_PHONE..."
            sudoclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! SudoClaw is down. Run: ssh l36 '~/sudoclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
