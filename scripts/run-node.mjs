...
const shouldBuild = (deps) => {
  if (deps.env.SUDOCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.SUDOCLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[sudoclaw] ${message}\n`);
};

const runSudoClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["sudoclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
