#!/usr/bin/env bash
# Rootless SudoClaw in Podman: run after one-time setup.
#
...
if [[ -n "${SUDOCLAW_UID:-}" && "$(id -u)" -eq "$SUDOCLAW_UID" ]]; then
  ...
  export HOME="$SUDOCLAW_HOME"
fi
if [[ -z "${EFFECTIVE_HOME:-}" ]]; then
  EFFECTIVE_HOME="${SUDOCLAW_HOME:-/tmp}"
fi
...
HOST_BRIDGE_PORT="${SUDOCLAW_PODMAN_BRIDGE_HOST_PORT:-${SUDOCLAW_BRIDGE_PORT:-18790}}"
...
if [[ -z "${SUDOCLAW_GATEWAY_TOKEN:-}" ]]; then
  export SUDOCLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "SUDOCLAW_GATEWAY_TOKEN" "$SUDOCLAW_GATEWAY_TOKEN"
  echo "Generated SUDOCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

# The gateway refuses to start unless gateway.mode=local is set in config.
# Keep this minimal; users can run the wizard later to configure channels/providers.
CONFIG_JSON="$CONFIG_DIR/sudoclaw.json"
...
PODMAN_USERNS="${SUDOCLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported SUDOCLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (SUDOCLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi
...
if [[ "$RUN_SETUP" == true ]]; then
  exec podman run --pull="$PODMAN_PULL" --rm -it \
    ...
    -e SUDOCLAW_GATEWAY_TOKEN="$SUDOCLAW_GATEWAY_TOKEN" \
    -v "$CONFIG_DIR:/home/node/.sudoclaw:rw" \
    -v "$WORKSPACE_DIR:/home/node/.sudoclaw/workspace:rw" \
    "${ENV_FILE_ARGS[@]}" \
    "$SUDOCLAW_IMAGE" \
